...
public:
    ...
    {
        AcquireGIL gil;
        throw_ifnull(PyObject_CallMethod(o, "operation_progress", "s#s#s#",
                                         ds.data(), ds.size(), operation.data(),
                                         operation.size(), message.data(),
                                         message.size()));
    }
    ...
    {
        AcquireGIL gil;
        throw_ifnull(PyObject_CallMethod(o, "operation_manual_intervention",
                                         "s#s#s#", ds.data(), ds.size(),
    ...
    {
        AcquireGIL gil;
        throw_ifnull(PyObject_CallMethod(o, "operation_aborted", "s#s#s#",
                                         ds.data(), ds.size(), operation.data(),
                                         operation.size(), message.data(),
                                         message.size()));
    }
    ...
    {
        AcquireGIL gil;
        throw_ifnull(PyObject_CallMethod(o, "operation_report", "s#s#s#",
                                         ds.data(), ds.size(), operation.data(),
                                         operation.size(), message.data(),
                                         message.size()));
    }
    ...
    {
        AcquireGIL gil;
        throw_ifnull(PyObject_CallMethod(o, "segment_info", "s#s#s#", ds.data(),
                                         ds.size(), relpath.native().data(),
                                         relpath.native().size(),
                                         message.data(), message.size()));
    }
                        ...
                        const std::string& message) override
    {
        ...
        throw_ifnull(PyObject_CallMethod(
            o, "segment_repack", "s#s#s#", ds.data(), ds.size(),
            relpath.native().data(), relpath.native().size(), message.data(),
            message.size()));
    }
                         ...
                         const std::string& message) override
    {
        AcquireGIL gil;
        throw_ifnull(PyObject_CallMethod(
            o, "segment_archive", "s#s#s#", ds.data(), ds.size(),
            relpath.native().data(), relpath.native().size(), message.data(),
            message.size()));
    }
                        ...
                        const std::string& message) override
    {
        AcquireGIL gil;
        throw_ifnull(PyObject_CallMethod(
            o, "segment_remove", "s#s#s#", ds.data(), ds.size(),
            relpath.native().data(), relpath.native().size(), message.data(),
            message.size()));
    }
                         ...
                         const std::string& message) override
    {
        AcquireGIL gil;
        throw_ifnull(PyObject_CallMethod(
            o, "segment_deindex", "s#s#s#", ds.data(), ds.size(),
            relpath.native().data(), relpath.native().size(), message.data(),
            message.size()));
    }
                        ...
                        const std::string& message) override
    {
        AcquireGIL gil;
        throw_ifnull(PyObject_CallMethod(
            o, "segment_rescan", "s#s#s#", ds.data(), ds.size(),
            relpath.native().data(), relpath.native().size(), message.data(),
            message.size()));
    }
    ...
    {
        AcquireGIL gil;
        throw_ifnull(PyObject_CallMethod(o, "segment_tar", "s#s#s#", ds.data(),
                                         ds.size(), relpath.native().data(),
                                         relpath.native().size(),
                                         message.data(), message.size()));
    }
                          ...
                          const std::string& message) override
    {
        AcquireGIL gil;
        throw_ifnull(PyObject_CallMethod(
            o, "segment_compress", "s#s#s#", ds.data(), ds.size(),
            relpath.native().data(), relpath.native().size(), message.data(),
            message.size()));
    }
                         ...
                         const std::string& message) override
    {
        AcquireGIL gil;
        throw_ifnull(PyObject_CallMethod(
            o, "segment_issue51", "s#s#s#", ds.data(), ds.size(),
            relpath.native().data(), relpath.native().size(), message.data(),
            message.size()));
    }
                                     ...
                                     const std::string& message) override
    {
        AcquireGIL gil;
        throw_ifnull(PyObject_CallMethod(
            o, "segment_manual_intervention", "s#s#s#", ds.data(), ds.size(),
            relpath.native().data(), relpath.native().size(), message.data(),
            message.size()));
    }
};
...
public:
    ...
    void write(const std::string& str)
    {
        AcquireGIL gil;
        throw_ifnull(
            PyObject_CallMethod(o, "write", "s#", str.data(), str.size()));
    }
                                       ...
                                       const std::string& message) override
    {
        std::stringstream out;
        out << ds << ": " << operation
            << " manual intervention required: " << message << std::endl;
        write(out.str());
    }
    void operation_aborted(const std::string& ds, const std::string& operation,
                           const std::string& message) override
