...
class QmacroTestMixin:
    ...
    def datasets(self, **kw):
        ...
        with Env(**kw) as env:
            dest = env.session.dataset_writer(cfg=env.ds_cfg)

            source = env.session.dataset_reader(cfg={
                "format": "grib",
                "name": "test.grib1",
...
class TestNoop(QmacroTestMixin, unittest.TestCase):
    def test_noop(self):
        ...
        with self.datasets() as env:
            with env.session.querymacro("noop", "testds") as dataset:
    ...
    def test_noopcopy(self):
        ...
        with self.datasets() as env:
            with env.session.querymacro("noopcopy", "testds") as dataset:
                with dataset.reader() as reader:
                    mdc = reader.query_data()
                    self.assertEqual(len(mdc), 9)
