...
void load_formatter_object()
{
    ...
    for (const auto& source : sources)
    {
        ...
        std::string module_name =
            "arkimet.formatter." + basename.substr(0, basename.size() - 3);
    ...
    pyo_unique_ptr module(
        throw_ifnull(PyImport_ImportModule("arkimet.formatter")));
    pyo_unique_ptr cls(
        throw_ifnull(PyObject_GetAttrString(module, "Formatter")));
    pyo_unique_ptr obj(throw_ifnull(PyObject_CallFunction(cls, nullptr)));

    // Hold a reference to arki.python.Formatter forever once loaded the first
    // time
...
struct PythonFormatter : public arki::Formatter
{
    std::string format(const arki::types::Type& v) const override
    {
        ...
        pyo_unique_ptr obj(
            PyObject_CallMethod(formatter_object, "format", "O", e.res.get()));
        ...
        if (!obj)
        {
            ...
            arki::nag::warning("python formatter failed: %s",
                               exc_type.str().c_str());
    ...
    static PyObject* run(Impl* self, PyObject* args, PyObject* kw)
    {
        ...
        try
        {
            ...
            std::unique_ptr<arki::types::Type> type =
                arki::types::decode_structure(arki::structured::keys_python,
                                              reader);
