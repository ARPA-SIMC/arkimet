...
#define ARKI_CATCH_RETURN_PYO                                                  \
    catch (arki::python::PythonException&) { return nullptr; }                 \
    ...
    {                                                                          \
        arki::python::set_std_exception(se);                                   \
        return nullptr;                                                        \
    }

#define ARKI_CATCH_RETURN_INT                                                  \
    catch (arki::python::PythonException&) { return -1; }                      \
    catch (std::invalid_argument & e)                                          \
    {                                                                          \
        PyErr_SetString(PyExc_ValueError, e.what());                           \
        return -1;                                                             \
...
#define ARKI_CATCH_RETHROW_PYTHON                                              \
    catch (arki::python::PythonException&) { throw; }                          \
    catch (std::invalid_argument & e)                                          \
    {                                                                          \
        PyErr_SetString(PyExc_ValueError, e.what());                           \
        throw arki::python::PythonException();                                 \
    }                                                                          \
    catch (std::exception & se)                                                \
    {                                                                          \
        arki::python::set_std_exception(se);                                   \
 ...
 */
template <typename Class> struct SharedPtrWrapper
{
    PyObject_HEAD std::shared_ptr<Class> ptr;
...
template <> inline DataFormat from_python<DataFormat>(PyObject* o)
{
    return dataformat_from_python(o);
}
...
template <> inline core::Time from_python<core::Time>(PyObject* o)
{
    return core_time_from_python(o);
}

/// Convert a DataFormat to a Python object
PyObject* core_time_to_python(const core::Time& time);
inline PyObject* to_python(const core::Time& val)
{
    return core_time_to_python(val);
}
