...
struct Output
{
    ...
    void open_ok_stdout()
    {
        output_ok = dballe::File::create(dballe::Encoding::BUFR, stdout, false,
                                         "[standard output]");
    }
    ...
    {
        output_ok =
            dballe::File::create(dballe::Encoding::BUFR, pathname, "wb");
    }
...
protected:
    ...
    void copy_base_msg(wreport::BufrBulletin& dst,
                       const wreport::BufrBulletin& src)
    {
        ...
        dst.master_table_version_number_local =
            src.master_table_version_number_local;
    ...
    void splitmsg(const dballe::BinaryMessage& rmsg,
                  const wreport::BufrBulletin& msg, dballe::Importer& importer,
                  Output& output)
    {
...
public:
    ...
    {
        auto file(dballe::File::create(dballe::Encoding::BUFR, stdin, false,
                                       "[standard input]")
                      .release());
        process(*file, output);
    }
    ...
    {
        auto file(
            dballe::File::create(dballe::Encoding::BUFR, filename.c_str(), "r")
                .release());
        process(*file, output);
    }
    ...
    {
        // Use .release() so the code is the same even with the new C++11's
        // dballe
        ...
        while (dballe::BinaryMessage rmsg = infile.read())
        {
            ...
            try
            {
                msg = wreport::BufrBulletin::decode(
            ...
            catch (std::exception& e)
            {
                arki::nag::warning("%s:%ld: BUFR #%d failed to decode: %s.",
                                   rmsg.pathname.c_str(), rmsg.offset,
                                   rmsg.index, e.what());
                decoded = false;
            }
...
struct run_ : public MethKwargs<run_, arkipy_ArkiBUFRPrepare>
{
    ...
    {
        static const char* kwlist[] = {"inputs", "usn", "outfile", "failfile",
                                       nullptr};
        ...
        if (!PyArg_ParseTupleAndKeywords(
                args, kw, "|OOz#z#", const_cast<char**>(kwlist), &arg_inputs,
                &arg_usn, &arg_outfile, &arg_outfile_len, &arg_failfile,
                &arg_failfile_len))
            return nullptr;
...
struct ArkiBUFRPrepareDef
    : public Type<ArkiBUFRPrepareDef, arkipy_ArkiBUFRPrepare>
{
