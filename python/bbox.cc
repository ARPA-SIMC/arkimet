...
void load_bbox_object()
{
    ...
    for (const auto& source : sources)
    {
        ...
        std::string module_name =
            "arkimet.bbox." + basename.substr(0, basename.size() - 3);
...
std::pair<double, double> get_coord_pair(PyObject* obj)
{
    ...
    if (size != 2)
        throw std::invalid_argument(
            "python bbox function did not return a list of coordinate pairs");
    PyObject* first  = PyTuple_GET_ITEM(obj, 0); // Borrowed reference
    PyObject* second = PyTuple_GET_ITEM(obj, 1); // Borrowed reference
    return std::make_pair(from_python<double>(first),
                          from_python<double>(second));
}

struct PythonBBox : public arki::BBox
{
    arki::utils::geos::Geometry
    compute(const arki::types::Area& v) const override
    {
        AcquireGIL gil;
        if (!bbox_object)
            load_bbox_object();
        ...
        pyo_unique_ptr obj(
            PyObject_CallMethod(bbox_object, "compute", "O", e.res.get()));
        ...
        if (!obj)
        {
            ...
            arki::nag::debug_tty("python bbox failed: %s",
                                 exc_type.str().c_str());
            arki::nag::warning("python bbox failed: %s",
                               exc_type.str().c_str());
        ...
        switch (size)
        {
            ...
            case 1: {
                PyObject* pair =
                    PyList_GET_ITEM(obj.get(), 0); // Borrowed reference
                auto point = get_coord_pair(pair);
                return arki::utils::geos::Geometry::create_point(point.first,
                                                                 point.second);
            }
            default: {
                ...
                {
                    PyObject* pair =
                        PyList_GET_ITEM(obj.get(), i); // Borrowed reference
                    auto point = get_coord_pair(pair);
                    seq.setxy(i, point.second, point.first);
                }
                arki::utils::geos::Geometry linear_ring(
                    arki::utils::geos::Geometry::create_linear_ring(
                        std::move(seq)));
                return arki::utils::geos::Geometry::create_polygon(
                    std::move(linear_ring),
                    arki::utils::geos::GeometryVector());
            }
        }
    }
};
...
struct compute : public MethKwargs<compute, arkipy_BBox>
{
    ...
    static PyObject* run(Impl* self, PyObject* args, PyObject* kw)
    {
        ...
        if (!PyArg_ParseTupleAndKeywords(args, kw, "O",
                                         const_cast<char**>(kwlist), &py_type))
            return nullptr;
        ...
        try
        {
            ...
            else
            {
                ...
                type = arki::types::decode_structure(
                    arki::structured::keys_python, reader);
            }
            std::unique_ptr<arki::types::Area> area =
                arki::downcast<arki::types::Area>(std::move(type));
...
BBox for arkimet metadata.
    ...
    static PyObject* _str(Impl* self) { return PyUnicode_FromString("BBox"); }

    static PyObject* _repr(Impl* self) { return PyUnicode_FromString("BBox"); }
