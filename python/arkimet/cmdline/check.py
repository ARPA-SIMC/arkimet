...
class Check(AppConfigMixin, App):
    ...
    read from config files), perform a check, fix, or cleanup run on them.
    ...
    def make_parser(cls) -> argparse.ArgumentParser:
        ...
        parser.add_argument("--restrict", action="store", metavar="names",
        ...
        parser.add_argument("--filter", action="store", metavar="matcher",
    ...
    def __init__(self, parser: argparse.ArgumentParser, args: argparse.Namespace):
        ...
        self.arki_check = arkimet.cmdline.ArkiCheck(
                self.session,
    ...
    def run(self):
        ...
        if self.args.remove:
            self.arki_check.remove(self.args.remove)
        elif self.args.remove_all:
            self.arki_check.remove_all()
        elif self.args.remove_old:
            self.arki_check.remove_old()
        elif self.args.repack:
            self.arki_check.repack()
        elif self.args.tar:
            self.arki_check.tar()
        elif self.args.zip:
            self.arki_check.zip()
        elif self.args.compress is not None:
            self.arki_check.compress(group_size=self.args.compress)
        elif self.args.unarchive is not None:
            self.arki_check.unarchive(pathname=self.args.unarchive)
        elif self.args.state:
            self.arki_check.state()
        elif self.args.issue51:
            self.arki_check.check_issue51()
        else:
            self.arki_check.check()
