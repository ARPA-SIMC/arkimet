...
class Xargs(App):
    ...
    def __init__(self):
        super().__init__()
        self.parser.add_argument("-i", "--input", metavar="file", action="append",
                                 help="read data from this file(s) instead of standard input"
                                      " (can be given more than once)")
        self.parser.add_argument("--max-args", "-n", metavar="count", type=int,
                                 help="group at most this amount of data items per command invocation")
        self.parser.add_argument("--max-size", "-s", metavar="size",
        ...
        self.parser.add_argument("--time-interval", metavar="interval",
                                 help="create one data file per 'interval', where interval can be minute,"
                                      " hour, day, month or year")
        self.parser.add_argument("--split-timerange", action="store_true",
