...
Archives are stored in the ``.archives`` subdirectory of a dataset. The
``archive age`` configuration value can be used to automate moving to
``.archives/``, during repack, those segments that only contain data older than
the given age.
...
``.archives/last`` dataset, and can be manually moved from ``last`` to any
other subdirectory of ``.archives``. arkimet will only ever move segments to
...
as ``.archives/last`` during maintenance. They are instances of :ref:`simple
