...
{
    return sys::access(root / "MANIFEST", F_OK) or
           sys::access(root / "index.sqlite", F_OK);
}
...
std::vector<SegmentInfo> read_plain(const std::filesystem::path& path)
{
    ...
    for (size_t lineno = 1; reader->getline(line); ++lineno)
    {
        ...
        res.emplace_back(SegmentInfo{
            file, mtime,
            core::Interval(core::Time::create_sql(line.substr(beg, end - beg)),
                           interval_end)});
    }
...
std::vector<SegmentInfo> read_sqlite(const std::filesystem::path& path)
{
    ...
    auto query = "SELECT file, mtime, start_time, end_time FROM files ORDER BY "
                 "start_time";
...
Reader::Reader(const std::filesystem::path& root) : m_root(root) {}
...
const SegmentInfo* Reader::segment(const std::filesystem::path& relpath) const
{
    ...
    auto lb = lower_bound(
        segmentinfo_cache.begin(), segmentinfo_cache.end(), sample,
        [](const auto& a, const auto& b) { return a.relpath < b.relpath; });
...
void Writer::set(const std::filesystem::path& relpath, time_t mtime,
                 const core::Interval& time)
{
    ...
    auto lb = std::lower_bound(
        segmentinfo_cache.begin(), segmentinfo_cache.end(), info,
        [](const auto& a, const auto& b) { return a.relpath < b.relpath; });
...
void Writer::set_mtime(const std::filesystem::path& relpath, time_t mtime)
{
    if (auto* info = segment(relpath))
    {
        ...
        throw std::runtime_error(
            relpath.native() +
            ": cannot update mtime since segment is not in index");
}

void Writer::remove(const std::filesystem::path& relpath)
{
    auto i =
        std::find_if(segmentinfo_cache.begin(), segmentinfo_cache.end(),
                     [&](const auto& el) { return el.relpath == relpath; });
...
void Writer::write(const SegmentInfo& info,
                   core::NamedFileDescriptor& out) const
{
    ...
    std::stringstream ss;
    ss << info.relpath.native() << ";" << info.mtime << ";"
       << info.time.begin.to_sql() << ";" << end.to_sql() << std::endl;
...
void Writer::flush()
{
    ...
    if (::rename(pathname.c_str(), target.c_str()) < 0)
        throw_system_error("cannot rename " + pathname.native() + " to " +
                           target.native());
...
void Writer::rename(const std::filesystem::path& relpath,
                    const std::filesystem::path& new_relpath)
{
    ...
    std::sort(
        segmentinfo_cache.begin(), segmentinfo_cache.end(),
        [](const auto& a, const auto& b) { return a.relpath < b.relpath; });
}
