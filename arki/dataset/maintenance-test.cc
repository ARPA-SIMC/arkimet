...
Fixture::Fixture(const std::string& format, const std::string& cfg_instance,
                 DatasetTest::TestVariant variant)
...
void Fixture::state_is(unsigned segment_count,
...
{
    makeSegmentedChecker()->test_make_overlap(test_relpath, test_datum_size / 2,
                                              1);
}
...
template <typename Fixture> CheckTest<Fixture>::~CheckTest() {}

template <typename Fixture> FixTest<Fixture>::~FixTest() {}

template <typename Fixture> RepackTest<Fixture>::~RepackTest() {}

template <typename Fixture> void CheckTest<Fixture>::register_tests_concat()
{
    // Check
        ...
        - the segment must be a file
                     ...
                     [&](Fixture& f) {
                         ...
                         wassert(actual(nag_messages.collected[0])
                                     .matches("W:.+2007/07-07\\.\\w+: segment "
...
template <typename Fixture> void CheckTest<Fixture>::register_tests_dir()
{
    this->add_method(
        "check_isdir", R"(
        ...
        [](const Fixture& f) {
            ...
            wassert_true(
                std::filesystem::is_directory("testds" / f.test_relpath));
        });
    ...
    this->add_method("check_ignore_dir_timestamp", R"(
       - the modification time of a directory segment can vary unpredictably,
         ...
         instead.
                     ...
                     [&](Fixture& f) {
                         sys::touch("testds" / f.test_relpath,
                                    time(NULL) + 86400);
...
template <typename Fixture> void CheckTest<Fixture>::register_tests_zip()
{
    // Check
    this->add_method(
        "check_iszip", R"(
        ...
        [](Fixture& f) {
            wassert(
                actual_file("testds" / sys::with_suffix(f.test_relpath, ".zip"))
                    .exists());
            wassert(actual_file("testds" / f.test_relpath).not_exists());
        });
}

template <typename TestFixture> void CheckTest<TestFixture>::register_tests()
{
        ...
        this->add_method("check_new", R"(
            - data files not known by a valid index are considered data files whose
              entire content has been removed [deleted]
                         ...
                         [&](Fixture& f) {
                             f.makeSegmentedChecker()->test_delete_from_index(
                                 f.test_relpath);
                             wassert(f.state_is(3, segment::SEGMENT_DELETED));
                         });
    ...
    this->add_method(
        "check_metadata_reftimes_must_fit_segment", R"(
    - the span of reference times in each segment must fit inside the interval
      ...
      archives, to deal with datasets that had a change of step in their lifetime?) [corrupted]
        ...
        [&](Fixture& f) {
            ...
            f.makeSegmentedChecker()->test_change_metadata(f.test_relpath, md,
                                                           0);
    ...
    if (can_detect_segments_out_of_step())
        this->add_method(
            "check_segment_name_must_fit_step", R"(
        - the segment name must represent an interval matching the dataset step
          ...
          a change of step in their lifetime?) [corrupted]
            ...
            [&](Fixture& f) {
                f.makeSegmentedChecker()->test_rename(f.test_relpath,
                                                      f.test_relpath_wrongstep);

                auto state = f.scan_state();
                wassert(actual(state.size()) == 3u);
                wassert(
                    actual(
                        state.get("testds:" + f.test_relpath_wrongstep.native())
                            .state) == segment::SEGMENT_CORRUPTED);

                // We are breaking the invariant that segments sorted by file
                // name are in the same sequence of segments sorted by time of
                // file contents, but the invariant that data queried is sorted
                // by reftime must still stand
                wassert(f.query_results({1, 3, 0, 2}));
            });
    ...
    this->add_method("check_unaligned", R"(
        - segments not known by the index, but when the index is either
          ...
          for reindexing instead of deletion [unaligned]
                     ...
                     [&](Fixture& f) {
                         f.makeSegmentedChecker()->test_invalidate_in_index(
                             f.test_relpath);
...
template <typename Fixture> void FixTest<Fixture>::register_tests_concat()
{
...
template <typename Fixture> void FixTest<Fixture>::register_tests_dir()
{
         ...
         sequence number found.
                     ...
                     [&](Fixture& f) {
...

                         {
                             auto checker(f.makeSegmentedChecker());
                             ReporterExpected e;
                             e.report.emplace_back("testds", "check",
                                                   "2 files ok");
    ...
    this->add_method(
        "fix_truncated_file_above_sequence", R"(
       - [unaligned] fix low sequence file value by setting it to the highest
...
template <typename TestFixture> void FixTest<TestFixture>::register_tests()
{
    switch (Fixture::segment_type())
    {
        case SEGMENT_CONCAT: register_tests_concat(); break;
    ...
    this->add_method(
        ...
        [&](Fixture& f) {
...

            {
                ...
                ReporterExpected e;
                e.report.emplace_back("testds", "check", "2 files ok");
                e.segment_manual_intervention.emplace_back("testds",
                                                           f.test_relpath);
                wassert(actual(checker.get()).check(e, true));
            }
...
template <typename Fixture> void RepackTest<Fixture>::register_tests_concat()
{
        ...
        // Do a repack, it should change the timestamp
        {
            ...
            auto segment =
                checker->dataset().segment_session->segment_from_relpath(
                    f.test_relpath);
            auto repack_result =
                wcallchecked(checker->segment(segment)->repack());
...
template <typename Fixture> void RepackTest<Fixture>::register_tests_dir() {}

template <typename TestFixture> void RepackTest<TestFixture>::register_tests()
{
    switch (Fixture::segment_type())
    {
        case SEGMENT_CONCAT: register_tests_concat(); break;
    ...
    this->add_method("repack_dirty", R"(
        - [dirty] segments are rewritten to be without holes and have data in the right order.
          ...
          with frequent appends and removes.
                     ...
                     [&](Fixture& f) {
                         f.make_hole_middle();
                         // swap_data(); // FIXME: swap_data currently isn't
                         // detected as dirty on simple datasets
    ...
    this->add_method(
        ...
        [&](Fixture& f) {
            ...
            wassert(actual_file("testds" / f.test_relpath).not_exists());
            wassert(
                actual_file("testds/.archive/last" / f.test_relpath_ondisk())
                    .exists());
            wassert(actual_file("testds/.archive/last" /
                                sys::with_suffix(f.test_relpath, ".metadata"))
                        .exists());
            wassert(actual_file("testds/.archive/last" /
                                sys::with_suffix(f.test_relpath, ".summary"))
                        .exists());
    ...
    this->add_method(
        ...
        [&](Fixture& f) {
            ...
            // State knows of a segment both to repack and to delete
            {
                ...
                wassert(
                    actual(
                        state.get("testds:" + f.test_relpath.native()).state) ==
                    segment::SEGMENT_DIRTY + segment::SEGMENT_DELETE_AGE);
        ...
        [&](Fixture& f) {
            ...
            // State knows of a segment both to repack and to delete
            {
                ...
                wassert(
                    actual(
                        state.get("testds:" + f.test_relpath.native()).state) ==
                    segment::SEGMENT_DIRTY + segment::SEGMENT_ARCHIVE_AGE);
                wassert(actual(state.count(segment::SEGMENT_OK)) == 2u);
                wassert(actual(state.size()) == 3u);
                state.all_ok();
            }
