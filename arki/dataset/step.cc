...
struct BaseFiles : public Files
{
    ...
    void list(std::function<void(std::filesystem::path&& relpath)> dest)
        const override
    {
        auto re = make_regexp();
        ...
        for (sys::Path::iterator f = dir.begin(); f != dir.end(); ++f)
        {
            if (f->d_name[0] == '.')
                continue;
            if (!re->match(f->d_name))
                continue;
            if (dirs.query.format != to_format(*re))
                continue;
...
struct SingleFiles : public Files
{
        ...
        const override
    {
        ...
        if (!std::filesystem::exists(dirs.query.root / relpath))
            return;
        dest(move(relpath));
    }
    ...
    core::Interval first() const override
    {
        return core::Interval(core::Time::create_lowerbound(1000),
...
struct YearFiles : public BaseFiles
{
    ...
    core::Interval to_period(const Regexp& re) const override
    {
        ...
        return core::Interval(core::Time::create_lowerbound(year),
                              core::Time::create_lowerbound(year + 1));
    }

    DataFormat to_format(const Regexp& re) const override
    {
        return format_from_string(re[3]);
    }
    std::filesystem::path to_relpath(const Regexp& re) const override
    {
        return re[1];
    }
};
...
struct MonthDayFiles : public BaseFiles
{
    ...
    std::unique_ptr<Regexp> make_regexp() const override
    {
        string re = "^(([[:digit:]]{2})-([[:digit:]]{2})\\.([^.]+))" +
                    dirs.query.extension_re;
    ...
    DataFormat to_format(const Regexp& re) const override
    {
        return format_from_string(re[4]);
    }
    std::filesystem::path to_relpath(const Regexp& re) const override
    {
        return re[1];
...
Dirs::Dirs(const SegmentQuery& query) : query(query) {}
...
struct BaseDirs : public Dirs
{
    ...
    virtual std::unique_ptr<Files> make_files(const std::filesystem::path& name,
                                              int value) const = 0;
...
struct SingleDirs : public Dirs
{
    ...
    void list(std::function<void(std::unique_ptr<Files>)> dest) const override
    {
        if (!std::filesystem::exists(query.root /
                                     ("all."s + format_name(query.format))))
            return;
    ...
    void extremes(core::Interval& first, core::Interval& last) const override
    {
        ...
        first = core::Interval(core::Time::create_lowerbound(1000),
                               core::Time::create_lowerbound(100000));
        last  = core::Interval(core::Time::create_lowerbound(1000),
                               core::Time::create_lowerbound(100000));
    }
};

template <typename FILES> struct CenturyDirs : public BaseDirs
{
    ...
    {
        return core::Interval(core::Time::create_lowerbound(value * 100),
                              core::Time::create_lowerbound((value + 1) * 100));
    }

    std::unique_ptr<Files> make_files(const std::filesystem::path& name,
...
template <typename FILES> struct YearDirs : public BaseDirs
{
    using BaseDirs::BaseDirs;
    ...
    core::Interval to_period(int value) const override
    {
        return core::Interval(core::Time::create_lowerbound(value),
...
struct StepQuery
{
    ...
    virtual void
    list_segments(const Matcher& m,
                  std::function<void(std::filesystem::path&&)> dest) const = 0;
...
struct PathQuery
{
    ...
    PathQuery(const std::filesystem::path& root, const std::string& format,
              const Matcher& m)
        : root(root), format(format)
    {
        if (!m.empty())
...
struct StepParser
{
    ...
    virtual bool parse_component(const PathQuery& q, unsigned depth,
                                 const char* name) = 0;
};

struct BaseStep : public Step
{
    std::unique_ptr<step::Dirs>
    explore(const step::SegmentQuery& query) const override
    {
        throw std::runtime_error("Step::explore not available for " +
                                 query.root.native());
    }

    void time_extremes(const step::SegmentQuery& query,
                       core::Interval& interval) const override
    {
    ...
    bool pathMatches(const std::filesystem::path& path,
                     const matcher::OR& m) const override
    {
    ...
    void list_segments(
        const step::SegmentQuery& query,
        std::function<void(std::filesystem::path&&)> dest) const override
    {
        auto dirs(explore(query));
        dirs->list(
            [&](std::unique_ptr<step::Files> files) { files->list(dest); });
    }
};
...
struct Single : public BaseStep
{
    ...
    bool path_timespan(const std::filesystem::path& path,
                       core::Interval& interval) const override
    {
...
struct Yearly : public BaseStep
{
    ...
    explore(const step::SegmentQuery& query) const override
    {
        return std::unique_ptr<step::Dirs>(
            new step::CenturyDirs<step::YearFiles>(query));
    }
...
struct Monthly : public BaseStep
{
    ...
    {
        return std::unique_ptr<step::Dirs>(
            new step::YearDirs<step::MonthFiles>(query));
    }
...
struct Daily : public BaseStep
{
    ...
    {
        return std::unique_ptr<step::Dirs>(
            new step::YearDirs<step::MonthDayFiles>(query));
    }
...
std::shared_ptr<Step> Step::create(const std::string& type)
{
    if (type == Daily::name())
        ...
        throw std::runtime_error("step '" + type +
                                 "' is not supported.  Valid values are daily, "
                                 "weekly, biweekly, monthly, and yearly.");
}

std::vector<std::filesystem::path> Step::list()
{
    std::vector<std::filesystem::path> res{
        Daily::name(),   Weekly::name(), Biweekly::name(),
        Monthly::name(), Yearly::name(),
    };
    return res;
}
