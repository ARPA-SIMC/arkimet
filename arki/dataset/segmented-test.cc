...
void Tests::register_tests()
{
    ...
    add_method("gz", [](Fixture& f) {
        ...
        // Check if files to archive are detected
        {
            ...
            wassert(actual(state.get("testds:2007/07-07.grib").state) ==
                    segment::SEGMENT_ARCHIVE_AGE);
            wassert(actual(state.get("testds:2007/07-08.grib").state) ==
                    segment::SEGMENT_ARCHIVE_AGE);
            wassert(actual(state.get("testds:2007/10-09.grib").state) ==
                    segment::SEGMENT_OK);
        }
        ...
        wassert(actual_segment("testds/.archive/last/2007/07-07.grib")
                    .exists({".gz", ".metadata", ".summary"}));
        wassert(actual_segment("testds/.archive/last/2007/07-08.grib")
                    .exists({".gz", ".metadata", ".summary"}));
        wassert(actual_segment("testds/.archive/last/2007/10-09.grib")
                    .not_exists());
        ...
        // Maintenance should now show a normal situation
        {
            ...
            e.report.emplace_back("testds.archives.last", "check",
                                  "2 files ok");
            wassert(actual(writer.get()).check(e, false));
        }

        // Perform full maintenance and check that things are still ok
        // afterwards
        {
            auto writer(f.makeSegmentedChecker());
    ...
    add_method("gzidx", [](Fixture& f) {
        // Import and compress all the files
        f.clean_and_import();
        // Do an extra round of import to have at least the two data per segment
        // needed to trigger indexing
        ...
        // Check that the files have been moved to the archive
        wassert(actual_segment("testds/.archive/last/2007/07-07.grib")
                    .exists({".gz", ".gz.idx", ".metadata", ".summary"}));
        wassert(actual_segment("testds/.archive/last/2007/07-08.grib")
                    .exists({".gz", ".gz.idx", ".metadata", ".summary"}));
        wassert(actual_segment("testds/.archive/last/2007/10-09.grib")
                    .not_exists());
    ...
    add_method("tarred", [](Fixture& f) {
        ...
        {
            auto checker(f.makeSegmentedChecker());
            checker->segments_tracked(
                [&](segmented::CheckerSegment& seg) { seg.tar(); });
        }
    ...
    add_method("zipped", [](Fixture& f) {
        ...
        // tar segments
        {
            auto checker(f.makeSegmentedChecker());
            checker->segments_tracked(
                [&](segmented::CheckerSegment& seg) { seg.zip(); });
        }
    ...
    add_method("query_lots", [](Fixture& f) {
        // Test querying with lots of data, to trigger on disk metadata
        // buffering
        using namespace arki::types;
        matcher::Parser parser;
        ...
        // Query all the dataset and make sure the results are in the right
        // order
        ...
        struct CheckReftimeSortOrder : public CheckSortOrder
        {
            ...
            {
                const reftime::Position* rt =
                    md.get<types::reftime::Position>();
        ...
        struct CheckAllSortOrder : public CheckSortOrder
        {
            uint64_t make_key(const Metadata& md) const override
            {
                ...
                auto time = rt->get_Position();
                const area::VM2* area =
                    dynamic_cast<const area::VM2*>(md.get(TYPE_AREA));
...

        {
            ...
            reader->query_data(
                dq, [&](std::shared_ptr<Metadata> md) { return cso.eat(md); });
            wassert(actual(cso.seen) == 16128u);
        }
    });
    ...
    add_method("unarchive_segment", [](Fixture& f) {
        ...
        wassert(actual_file("testds/.archive/last/2007/07-07.grib.metadata")
                    .exists());
        wassert(actual_file("testds/.archive/last/2007/07-07.grib.summary")
                    .exists());
        wassert(
            actual_file("testds/.archive/last/2007/07-08.grib").not_exists());
        wassert(actual_file("testds/.archive/last/2007/07-08.grib.metadata")
                    .not_exists());
        wassert(actual_file("testds/.archive/last/2007/07-08.grib.summary")
                    .not_exists());
        wassert(
            actual_file("testds/.archive/last/2007/10-09.grib").not_exists());
        wassert(actual_file("testds/.archive/last/2007/10-09.grib.metadata")
                    .not_exists());
        wassert(actual_file("testds/.archive/last/2007/10-09.grib.summary")
                    .not_exists());
        ...
        wassert(
            actual_file("testds/.archive/last/2007/07-07.grib").not_exists());
        wassert(actual_file("testds/.archive/last/2007/07-07.grib.metadata")
                    .not_exists());
        wassert(actual_file("testds/.archive/last/2007/07-07.grib.summary")
                    .not_exists());
    ...
    add_method("scan_dir_empty", [](Fixture& f) {
        ...
        wassert(checker->scan_dir(
            [&](auto segment) { res.push_back(segment->relpath()); }));
    ...
    add_method("no_writes_when_repacking", [](Fixture& f) {
        ...
        test_hooks->on_check_lock = [&](const Segment& segment) {
            ...
            unsigned count = 0;
            reader->query_data(parser.parse("reftime:=2007-07-07"),
                               [&](const auto&) noexcept {
                                   ++count;
                                   return true;
                               });
...
Tests103 test103_simple("arki_dataset_segmented_issue103_simple",
                        "type=simple");
Tests103 test103_simple_dir("arki_dataset_segmented_issue103_simple_dir",
                            "type=simple", DatasetTest::TEST_FORCE_DIR);
Tests103 test103_iseg("arki_dataset_segmented_issue103_iseg",
                      "type=iseg\nformat=vm2\n");
Tests103 test103_iseg_dir("arki_dataset_segmented_issue103_iseg_dir",
                          "type=iseg\nformat=vm2\n",
...
void Tests103::register_tests()
{
    ...
    add_method("issue103", [](Fixture& f) {
        ...
        // TODO: speed up by lowering RLIMIT_NOFILE for the duration of this
        // test
        ...
        // Query with data, sorting, without storing all the results on a
        // collection
