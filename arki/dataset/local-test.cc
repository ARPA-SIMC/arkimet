...
postprocess = singlepoint, subarea, seriet, cat
        ...
        auto config =
            dataset::local::Reader::read_config("testds/.archive/last");
        wassert(actual(config->value("description")) == "test dataset");
        wassert(actual(config->value("filter")) == "origin:GRIB1,80,,22");
        wassert(actual(config->value("index")) ==
                "reftime, timerange, level, product");
        ...
        wassert(actual(config->value("unique")) ==
                "origin, reftime, timerange, level, product");
        wassert(actual(config->value("postprocess")) ==
                "singlepoint, subarea, seriet, cat");
        wassert(actual(config->value("name")) == "last");
        wassert(actual(config->value("path")) ==
                std::filesystem::canonical("testds/.archive/last"));
