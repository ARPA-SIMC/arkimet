...
void Tests::register_tests()
{
    ...
    add_method("read_config", [] {
        auto cfg =
            wcallchecked(dataset::Session::read_config("inbound/test.grib1"));
        wassert(actual(cfg->value("name")) == "inbound/test.grib1");
        wassert(actual(cfg->value("path")) ==
                std::filesystem::canonical("inbound/test.grib1"));

        cfg = wcallchecked(
            dataset::Session::read_config("grib:inbound/test.grib1"));
        wassert(actual(cfg->value("name")) == "inbound/test.grib1");
        wassert(actual(cfg->value("path")) ==
                std::filesystem::canonical("inbound/test.grib1"));
        ...
        wassert_throws(std::runtime_error,
                       dataset::Session::read_config("testds/file.unknown"));
    });
