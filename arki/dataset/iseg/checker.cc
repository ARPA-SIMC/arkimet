...
public:
    ...
    CIndex& idx()
    {
        return static_pointer_cast<segment::iseg::Checker>(segment_checker)
            ->index();
    }
    ...
    std::function<void(std::shared_ptr<const Segment>)> dest)
{
    ...
    for (const auto& relpath : seg_relpaths)
        dest(dataset().segment_session->segment_from_relpath_and_format(
            relpath, format));
}
    ...
    std::function<void(segmented::CheckerSegment& segment)> dest)
{
    step::SegmentQuery squery(dataset().path,
                              dataset().iseg_segment_session->format, matcher);
    dataset().step().list_segments(
        squery, [&](std::filesystem::path&& relpath) {
            auto segment =
                dataset().segment_session->segment_from_relpath(relpath);
            auto lock = dataset().check_lock_segment(relpath);
            if (sys::stat(segment->abspath_iseg_index()))
                return;
            CheckerSegment csegment(*this, segment, lock);
...
void Checker::check_issue51(CheckerConfig& opts)
{
    ...
    list_segments([&](std::shared_ptr<const Segment> segment) {
        ...
        nag::verbose(
            ...
            name().c_str(), segment->relpath().c_str(), count_ok,
...
size_t Checker::vacuum(dataset::Reporter&) { return 0; }
