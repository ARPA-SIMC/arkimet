    ...
    void register_tests() override
    {
        add_method("compressed", [](Fixture& f) {
            ...
            // Check if files to archive are detected
            {
                ...
                ensure_equals(c.count(DatasetTest::COUNTED_OK), 1u);
                ensure_equals(c.count(DatasetTest::COUNTED_TO_ARCHIVE), 2u);
            ...
            // Maintenance should now show a normal situation
            {
                ...
                writer->maintenance(c);
                ensure_equals(c.count(DatasetTest::COUNTED_OK), 1u);
                ensure_equals(c.count(DatasetTest::COUNTED_ARC_OK), 2u);
                ensure_equals(c.remaining(), "");
                ensure(c.isClean());
            }
...
Tests test_ondisk2("arki_dataset_local_ondisk2", "type=ondisk2\n");
Tests test_simple_plain("arki_dataset_local_simple_plain", "type=simple\nindex_type=plain\n");
Tests test_simple_sqlite("arki_dataset_local_simple_sqlite", "type=simple\nindex_type=sqlite");
