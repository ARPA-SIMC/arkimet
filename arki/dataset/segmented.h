...
public:
    ...
    /// What replace strategy to use when acquire() is called with
    /// REPLACE_DEFAULT
    ...
    virtual bool relpath_timespan(const std::filesystem::path& path,
...
protected:
    ...
    batch_by_segment(metadata::InboundBatch& batch);
...
public:
    ...
    // TODO: make it a member and refactor TestDispatcher to instantiate dataset
    // writers
    static void test_acquire(std::shared_ptr<Session>,
                             const core::cfg::Section& cfg,
...
struct SegmentState
{
    ...
    explicit SegmentState(arki::segment::State state) : state(state) {}
    ...
    void check_age(const std::filesystem::path& relpath, const Dataset& cfg,
...
protected:
    ...
    virtual void post_repack(std::shared_ptr<segment::Fixer>,
                             segment::Fixer::ReorderResult&)
    {
    }
    virtual void post_remove_data(std::shared_ptr<segment::Fixer>,
                                  segment::Fixer::MarkRemovedResult&)
    {
    }
    virtual void pre_remove(std::shared_ptr<segment::Fixer>) {}
    virtual void post_convert(std::shared_ptr<segment::Fixer>,
                              segment::Fixer::ConvertResult&)
    {
    }
...
public:
    ...
    virtual SegmentState fsck(dataset::Reporter& reporter,
                              bool quick = true) = 0;
     ...
     */
    virtual segment::Fixer::MarkRemovedResult
    remove_data(const std::set<uint64_t>& offsets);
...
protected:
    ...
    segment_from_relpath(const std::filesystem::path& relpath);
...
public:
    ...
    virtual std::unique_ptr<CheckerSegment>
    segment(std::shared_ptr<const Segment> segment) = 0;

    /// Instantiate a CheckerSegment using an existing lock
    virtual std::unique_ptr<CheckerSegment>
    segment_prelocked(std::shared_ptr<const Segment> segment,
                      std::shared_ptr<core::CheckLock> lock) = 0;

    /**
     * List all segments known to this dataset
     */
    virtual void
        segments_tracked(std::function<void(CheckerSegment& segment)>) = 0;

    /**
     * List all segments known to this dataset
     */
    virtual void segments_tracked_filtered(
        const Matcher& matcher,
        std::function<void(segmented::CheckerSegment& segment)>) = 0;

    /**
     * List all segments present on disk but not known to this dataset
     */
    virtual void segments_untracked(
        std::function<void(segmented::CheckerSegment& segment)>) = 0;

    /**
     * List all segments known to this dataset
     */
    virtual void segments_untracked_filtered(
        const Matcher& matcher,
        std::function<void(segmented::CheckerSegment& segment)>) = 0;

    /**
     * List all segments, both known to this dataset or unknown but found on
     * disk
     */
    void segments(CheckerConfig& config,
                  std::function<void(segmented::CheckerSegment& segment)>);
     ...
     */
    virtual void test_make_overlap(const std::filesystem::path& relpath,
     ...
     * This is used to simulate anomalies in the dataset during tests.
     */
    virtual void test_make_hole(const std::filesystem::path& relpath,
     ...
     * This is used to simulate anomalies in the dataset during tests.
     */
    virtual void test_corrupt_data(const std::filesystem::path& relpath,
                                   unsigned data_idx = 0);
     ...
     * This is used to simulate anomalies in the dataset during tests.
     */
    virtual void test_truncate_data(const std::filesystem::path& relpath,
                                    unsigned data_idx = 0);
     ...
     * This is used to simulate anomalies in the dataset during tests.
     */
    virtual void test_swap_data(const std::filesystem::path& relpath,
     ...
     * This is used to simulate anomalies in the dataset during tests.
     */
    virtual void test_rename(const std::filesystem::path& relpath,
     ...
     * This is used to simulate anomalies in the dataset during tests.
     */
    virtual metadata::Collection
    test_change_metadata(const std::filesystem::path& relpath,
                         std::shared_ptr<Metadata> md, unsigned data_idx = 0);
     ...
     */
    virtual void
    test_invalidate_in_index(const std::filesystem::path& relpath) = 0;
     ...
     */
    void
    scan_dir(std::function<void(std::shared_ptr<const Segment> segment)> dest);
};
