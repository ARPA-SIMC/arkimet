...
struct SegmentQuery
{
    ...
    SegmentQuery(const std::filesystem::path& root, DataFormat format,
                 const std::string& extension_re, const Matcher& matcher);
};
...
public:
     ...
     */
    virtual void
    list(std::function<void(std::unique_ptr<Files>)> dest) const = 0;

    /**
     * Return the intervals of the first and last segment under this directory.
     */
    virtual void extremes(core::Interval& first,
                          core::Interval& last) const = 0;
};
...
public:
    ...
    /**
     * List all matching segments in this directory, with paths relative to
     * dirs.root
     */
    virtual void
    list(std::function<void(std::filesystem::path&& relpath)> dest) const = 0;

    /**
     * Return the first segment in this directory, with path relative to
     * dirs.root
     */
    virtual core::Interval first() const = 0;

    /**
     * Return the last segment in this directory, with path relative to
     * dirs.root
     */
...
public:
    ...
    /**
     * Instantiate a step::Dirs to query segments for the given directory and
     * format
     */
    virtual std::unique_ptr<step::Dirs>
    explore(const step::SegmentQuery& query) const = 0;
     ...
     */
    virtual bool path_timespan(const std::filesystem::path& path,
                               core::Interval& interval) const = 0;
     ...
     */
    virtual bool pathMatches(const std::filesystem::path& path,
                             const matcher::OR& m) const = 0;
     ...
     */
    virtual void
    list_segments(const step::SegmentQuery& query,
     ...
     * segment.
     */
    virtual void time_extremes(const step::SegmentQuery& query,
                               core::Interval& interval) const = 0;
