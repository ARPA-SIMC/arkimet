...
struct Fixture : public arki::utils::tests::Fixture
{
    ...
    {
        auto writer =
            session->dataset(*config->section(dsname))->create_writer();
...
void Tests::register_tests()
{
    ...
    add_method("progress_query_data", [](Fixture& f) {
        ...
        size_t count = 0;
        reader->query_data(dq, [&](std::shared_ptr<Metadata> md) noexcept {
            ++count;
            return true;
        });
    ...
    add_method("progress_query_bytes_throws", [](Fixture& f) {
        ...
        auto e =
            wassert_throws(std::logic_error, reader->query_bytes(bq, *out));
        wassert(actual(e.what()) = "Expected error");
    });
}
