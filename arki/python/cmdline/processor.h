...
struct ProcessorMaker
{
    ...
    std::unique_ptr<DatasetProcessor> make(Matcher matcher,
                                           std::shared_ptr<StreamOutput> out);

    static std::unique_ptr<DatasetProcessor>
    make_libarchive(Matcher matcher, std::shared_ptr<StreamOutput> out,
                    std::string archive,
                    std::shared_ptr<arki::query::Progress> progress);

protected:
    std::unique_ptr<DatasetProcessor>
    make_binary(Matcher matcher, std::shared_ptr<StreamOutput> out);
    std::unique_ptr<DatasetProcessor>
    make_summary(Matcher matcher, std::shared_ptr<StreamOutput> out);
    std::unique_ptr<DatasetProcessor>
    make_metadata(Matcher matcher, std::shared_ptr<StreamOutput> out);
};
