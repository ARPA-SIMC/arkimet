...
static std::string moveFile(const std::filesystem::path& source,
                            const std::filesystem::path& targetdir)
{
    auto targetFile = targetdir / source.filename();
    if (::rename(source.c_str(), targetFile.c_str()) == -1)
        arki::throw_system_error(errno, "cannot move ", source, " to ",
                                 targetFile);
    return targetFile;
}

static std::string moveFile(const arki::dataset::Reader& reader,
                            const std::filesystem::path& targetdir)
{
    const auto& dataset = reader.dataset();

    if (const arki::dataset::file::SegmentDataset* ds =
            dynamic_cast<const arki::dataset::file::SegmentDataset*>(&dataset))
        return moveFile(ds->segment->abspath(), targetdir);
    else if (const arki::dataset::file::FdFile* ds =
                 dynamic_cast<const arki::dataset::file::FdFile*>(&dataset))
 ...
 */
std::unique_ptr<arki_scan::MetadataDispatch>
build_dispatcher(std::shared_ptr<arki::dataset::Pool> pool,
                 cmdline::DatasetProcessor& processor, PyObject* args,
                 PyObject* kw)
{
    static const char* kwlist[] = {
        "copyok",       "copyko",          "validate", "dispatch",
        "testdispatch", "flush_threshold", nullptr};
    ...
    if (!PyArg_ParseTupleAndKeywords(
            args, kw, "|z#z#z#OOK", const_cast<char**>(kwlist), &copyok,
            &copyok_len, &copyko, &copyko_len, &validate, &validate_len,
            &dispatch, &testdispatch, &flush_threshold))
        throw PythonException();

    std::unique_ptr<arki_scan::MetadataDispatch> res(
        new arki_scan::MetadataDispatch(pool, processor));

    if (testdispatch)
    {
        res->dispatcher =
            new arki::TestDispatcher(pool_from_python(testdispatch));
    }
    else if (dispatch)
    {
        ...
        throw std::runtime_error("cannot create MetadataDispatch with no "
                                 "--dispatch or --testdispatch information");

    if (validate)
    {
        const arki::metadata::ValidatorRepository& vals =
            arki::metadata::ValidatorRepository::get();
        ...
        {
            arki::metadata::ValidatorRepository::const_iterator i =
                vals.find(*iname);
            if (i == vals.end())
                throw std::runtime_error(
                    "unknown validator '" + *iname +
                    "'. You can get a list using --validate=list.");
...
struct set_dispatcher : public MethKwargs<set_dispatcher, arkipy_ArkiScan>
{
    ...
    static PyObject* run(Impl* self, PyObject* args, PyObject* kw)
    {
        ...
        {
            auto dispatcher =
                build_dispatcher(self->pool, *(self->processor), args, kw);
        ...
        try
        {
            bool all_successful;
            {
                ...
                all_successful = foreach_file(
                    self->pool->session(), in, dataformat_from_python(format),
                    [&](arki::dataset::Reader& reader) {
...
struct scan_sections : public MethKwargs<scan_sections, arkipy_ArkiScan>
{
    ...
    static PyObject* run(Impl* self, PyObject* args, PyObject* kw)
    {
        ...
        try
        {
            ...
            {
                ReleaseGIL rg;
                all_successful = foreach_sections(
                    self->pool, [&](arki::dataset::Reader& reader) {
                        self->processor->process(reader, reader.name());
                    });
                self->processor->end();
            }
...
static int
compute_result(bool all_processed, bool ignore_duplicates,
               const std::vector<arki_scan::DispatchResults>& results)
{
...
struct dispatch_file : public MethKwargs<dispatch_file, arkipy_ArkiScan>
{
    ...
    static PyObject* run(Impl* self, PyObject* args, PyObject* kw)
    {
        static const char* kwlist[] = {"file", "format", "ignore_duplicates",
                                       "status", nullptr};
        ...
        int status            = 0;
        if (!PyArg_ParseTupleAndKeywords(args, kw, "OO|pp",
                                         const_cast<char**>(kwlist), &file,
                                         &format, &ignore_duplicates, &status))
            return nullptr;
        ...
        try
        {
            ...
            bool all_processed = false;
            {
                ...
                all_processed = foreach_file(
                    self->pool->session(), in, dataformat_from_python(format),
                    [&](arki::dataset::Reader& reader) {
                        auto stats =
                            self->dispatcher->process(reader, reader.name());

                        if (status)
                            arki::nag::warning("%s: %s", stats.name.c_str(),
                                               stats.summary().c_str());
            ...
            return throw_ifnull(PyLong_FromLong(
                compute_result(all_processed, ignore_duplicates, results)));
        }
        ARKI_CATCH_RETURN_PYO
    }
};
...
struct dispatch_sections : public MethKwargs<dispatch_sections, arkipy_ArkiScan>
{
    ...
    static PyObject* run(Impl* self, PyObject* args, PyObject* kw)
    {
        static const char* kwlist[] = {"moveok",   "moveko",
                                       "movework", "ignore_duplicates",
                                       "status",   nullptr};
        ...
        int status            = 0;
        if (!PyArg_ParseTupleAndKeywords(
                args, kw, "|z#z#z#pp", const_cast<char**>(kwlist), &moveok,
                &moveok_len, &moveko, &moveko_len, &movework, &movework_len,
        ...
        try
        {
...

            {
                ...
                all_processed = self->pool->foreach_dataset(
                    [&](std::shared_ptr<arki::dataset::Dataset> ds) {
                        FileSource source(ds);
                        if (movework)
                            source.movework =
                                std::string(movework, movework_len);
                        if (moveok)
                            source.moveok = std::string(moveok, moveok_len);
                        if (moveko)
                            source.moveko = std::string(moveko, moveko_len);

                        source.open();
                        arki::nag::verbose("Processing %s...",
                                           source.reader->name().c_str());
                        bool success = true;
                        try
                        {
                            auto stats = self->dispatcher->process(
                                *source.reader, source.reader->name());
                            if (status)
                                arki::nag::warning("%s: %s", stats.name.c_str(),
                                                   stats.summary().c_str());
...
arki-scan implementation
    ...
    Methods<set_processor, set_dispatcher, scan_file, scan_sections,
            dispatch_file, dispatch_sections>
        methods;
