...
#include "arki/python/cfg.h"
#include "arki/python/common.h"
#include "arki/python/dataset/reporter.h"
#include "arki/python/matcher.h"
#include "arki/python/utils/dict.h"
...
static arki::dataset::CheckerConfig
get_checker_config(std::shared_ptr<arki::dataset::Session> session,
                   PyObject* args, PyObject* kw)
{
    static const char* kwlist[]  = {"reporter", "segment_filter", "offline",
                                    "online",   "readonly",       "accurate",
                                    nullptr};
    ...
    int arg_readonly             = 1;
    int arg_accurate             = 0;
    if (!PyArg_ParseTupleAndKeywords(args, kw, "|OOpppp",
                                     const_cast<char**>(kwlist), &arg_reporter,
                                     &arg_segment_filter, &arg_offline,
                                     &arg_online, &arg_readonly, &arg_accurate))
        throw PythonException();

    arki::dataset::CheckerConfig cfg;
    if (arg_reporter)
        cfg.reporter =
            make_shared<python::dataset::ProxyReporter>(arg_reporter);
...
struct repack : public MethKwargs<repack, arkipy_DatasetChecker>
{
    ...
    constexpr static const char* signature =
        "reporter: Any=None, segment_filter: Union[arkimet.Matcher, str]="
        ", offline: bool=True, online: bool=True, readonly: bool=True, "
        "accurate: bool=False";
    ...
    static PyObject* run(Impl* self, PyObject* args, PyObject* kw)
    {
        ...
        {
            auto cfg =
                get_checker_config(self->ptr->dataset().session, args, kw);
...
struct segment_state : public MethKwargs<segment_state, arkipy_DatasetChecker>
{
    ...
    constexpr static const char* signature =
        "reporter: Any=None, segment_filter: Union[arkimet.Matcher, str]="
        ", offline: bool=True, online: bool=True, readonly: bool=True, "
        "accurate: bool=False, time_override: int=None";
    ...
    static PyObject* run(Impl* self, PyObject* args, PyObject* kw)
    {
        try
        {
            ...
            arki::dataset::segmented::Checker* checker =
                dynamic_cast<arki::dataset::segmented::Checker*>(
                    self->ptr.get());
            if (!checker)
                Py_RETURN_NONE;
            ...
            {
                ReleaseGIL gil;
                checker->segments_recursive(
                    cfg,
                    [&](arki::dataset::segmented::Checker& checker,
                        arki::dataset::segmented::CheckerSegment& segment) {
                        std::string key = checker.name() + ":" +
                                          segment.path_relative().native();
                        auto state = segment.fsck(*cfg.reporter, !cfg.accurate);
                        AcquireGIL gil;
...
Examples::
    ...
    Methods<MethGenericEnter<Impl>, MethGenericExit<Impl>, repack, check,
            segment_state>
        methods;
    ...
    {
        if (self->ptr.get())
            return PyUnicode_FromFormat("dataset.Checker(%s, %s)",
                                        self->ptr->type().c_str(),
                                        self->ptr->name().c_str());
        else
    ...
    static int _init(Impl* self, PyObject* args, PyObject* kw)
    {
        ...
        try
        {
            ...
            if (PyErr_WarnEx(
                    ...
                    "Use arki.dataset.Session().dataset_checker(cfg=cfg) "
                    "instead of arki.dataset.Checker(cfg)",
                    1))
                return -1;

            auto session = std::make_shared<arki::dataset::Session>();
            new (&(self->ptr)) std::shared_ptr<arki::dataset::Checker>(
                session->dataset(*cfg)->create_checker());
            return 0;
        }
...
{
    arkipy_DatasetChecker* result =
        PyObject_New(arkipy_DatasetChecker, arkipy_DatasetChecker_Type);
    if (!result)
        return nullptr;
