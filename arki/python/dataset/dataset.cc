...
#include "arki/python/dataset/checker.h"
#include "arki/python/dataset/reader.h"
#include "arki/python/dataset/writer.h"
...
struct config : public Getter<config, arkipy_DatasetDataset>
{
    ...
    constexpr static const char* doc =
        "dataset configuration as an :class:`arkimet.cfg.Section` object";
    constexpr static void* closure = nullptr;
...
struct checker : public MethNoargs<checker, arkipy_DatasetDataset>
{
    ...
    static PyObject* run(Impl* self)
    {
        try
        {
            return (PyObject*)dataset_checker_create(
                self->ptr->create_checker());
        }
        ARKI_CATCH_RETURN_PYO
    }
};
...
Examples::
    ...
    Methods<MethGenericEnter<Impl>, MethGenericExit<Impl>, reader, writer,
            checker>
        methods;
    ...
    {
        return PyUnicode_FromFormat("dataset.Dataset(%s)",
                                    self->ptr->name().c_str());
    }
    ...
    static int _init(Impl* self, PyObject* args, PyObject* kw)
    {
        PyErr_SetString(PyExc_NotImplementedError,
                        "arki.dataset.Session.dataset() is currently the only "
                        "supported way to istantiate Dataset objects");
        return -1;
    }
};
...
arkipy_DatasetDataset*
dataset_dataset_create(std::shared_ptr<arki::dataset::Dataset> ptr)
{
    arkipy_DatasetDataset* result =
        PyObject_New(arkipy_DatasetDataset, arkipy_DatasetDataset_Type);
    if (!result)
        return nullptr;
...
std::shared_ptr<arki::dataset::Dataset> dataset_from_python(PyObject* o)
{
    try
    {
        ...
        PyErr_SetString(PyExc_TypeError,
                        "value must be an instance of arkimet.dataset.Dataset");
