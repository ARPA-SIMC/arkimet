...
struct reference_time : public Getter<reference_time, arkipy_Summary>
{
    ...
    constexpr static const char* doc =
        "Return a tuple of datetimes with the minimum and maximum known "
        "reference times";
    constexpr static void* closure = nullptr;
...
struct add : public MethKwargs<add, arkipy_Summary>
{
    ...
    constexpr static const char* signature =
        "val: Union[arki.Metadata, arki.Summary]";
    ...
    static PyObject* run(Impl* self, PyObject* args, PyObject* kw)
    {
        ...
        if (!PyArg_ParseTupleAndKeywords(args, kw, "O",
                                         const_cast<char**>(kwlist), &arg_val))
            return nullptr;
        ...
        try
        {
            ...
            else
            {
                PyErr_SetString(
                    PyExc_TypeError,
                    "Argument must be arki.Metadata or arki.Summary");
                return nullptr;
            }
            Py_RETURN_NONE;
        }
...
struct write : public MethKwargs<write, arkipy_Summary>
{
    ...
    constexpr static const char* signature =
        "file: Union[int, BinaryIO], format: str = 'binary', annotate: bool = "
        "False";
...
:param annotate: if True, use a :class:`arkimet.Formatter` to add metadata
    ...
    static PyObject* run(Impl* self, PyObject* args, PyObject* kw)
    {
        ...
        if (!PyArg_ParseTupleAndKeywords(args, kw, "O|sp",
        ...
        try
        {
            std::unique_ptr<StreamOutput> out =
            ...
            else
            {
                PyErr_Format(PyExc_ValueError,
                             "Unsupported metadata serialization format: %s",
                             format);
                return nullptr;
            }
            Py_RETURN_NONE;
...
struct get_convex_hull : public MethNoargs<get_convex_hull, arkipy_Summary>
{
    ...
    constexpr static const char* summary =
        "compute the convex hull for this summary and return it as a WKT "
        "string";
    constexpr static const char* doc = R"(
    ...
    static PyObject* run(Impl* self)
    {
        try
        {
            ...
            if (bbox)
            {
                ...
                pyo_unique_ptr res =
                    arki::python::to_python(writer.write(bbox));
                return res.release();
            }
            Py_RETURN_NONE;
        }
...
Examples::
    ...
    Methods<add, write, write_short, to_python, get_convex_hull, read_binary,
            read_yaml, read_json>
        methods;
...
{
    return (arkipy_Summary*)throw_ifnull(
        PyObject_CallObject((PyObject*)arkipy_Summary_Type, NULL));
}
