...
public:
    ...
    stream::SendResult send_file_segment(arki::core::NamedFileDescriptor& fd,
...
struct MockConcreteSyscalls
{
    ...
    std::function<ssize_t(int fd, const struct iovec* iov, int iovcnt)>
        orig_writev;
    std::function<ssize_t(int out_fd, int in_fd, off_t* offset, size_t count)>
        orig_sendfile;
...
struct ExpectedSyscallMatch
{
    ...
    ExpectedSyscallMatch(const std::string& name) : name(name) {}
    ...
    virtual ssize_t on_read(int fd, void* buf, size_t count)
    {
        fail_mismatch("read");
    }
    virtual ssize_t on_write(int fd, const void* buf, size_t count)
    {
        fail_mismatch("write");
    }
    virtual ssize_t on_writev(int fd, const struct iovec* iov, int iovcnt)
    {
        fail_mismatch("writev");
    }
    virtual ssize_t on_sendfile(int out_fd, int in_fd, off_t* offset,
                                size_t count)
    {
        fail_mismatch("sendfile");
    }
    virtual ssize_t on_splice(int fd_in, loff_t* off_in, int fd_out,
                              loff_t* off_out, size_t len, unsigned int flags)
    {
        fail_mismatch("splice");
    }
    virtual int on_poll(struct pollfd* fds, nfds_t nfds, int timeout)
    {
        fail_mismatch("poll");
    }
    virtual ssize_t on_pread(int fd, void* buf, size_t count, off_t offset)
    {
        fail_mismatch("pread");
    }
};
...
struct ExpectedRead : public ExpectedSyscallMatch
{
    ...
    ExpectedRead(int fd, const std::string& result, size_t count, ssize_t res,
                 int errno_val = 0)
        : ExpectedSyscallMatch("read"), fd(fd), result(result), count(count),
          res(res), errno_val(errno_val)
    {
    }
...
struct ExpectedPread : public ExpectedSyscallMatch
{
    ...
    ExpectedPread(int fd, off_t offset, size_t count, const std::string& result,
                  ssize_t res, int errno_val = 0)
        : ExpectedSyscallMatch("pread"), fd(fd), offset(offset), count(count),
          result(result), res(res), errno_val(errno_val)
    {
    }
...
struct ExpectedWrite : public ExpectedSyscallMatch
{
    ...
    ExpectedWrite(int fd, const std::string& expected, ssize_t res,
                  int errno_val = 0)
        : ExpectedSyscallMatch("write"), fd(fd), expected(expected), res(res),
          errno_val(errno_val)
    {
    }
...
struct ExpectedWritev : public ExpectedSyscallMatch
{
    ...
    ExpectedWritev(int fd, const std::vector<std::string>& expected,
                   ssize_t res, int errno_val = 0)
        : ExpectedSyscallMatch("writev"), fd(fd), expected(expected), res(res),
          errno_val(errno_val)
    {
    }
    ...
    ssize_t on_writev(int fd, const struct iovec* iov, int iovcnt) override
    {
        ...
        for (unsigned i = 0; i < (unsigned)iovcnt; ++i)
            wassert(actual(std::string((const char*)iov[i].iov_base,
                                       iov[i].iov_len)) == expected[i]);
        errno = errno_val;
        return res;
    }
};
...
struct ExpectedPoll : public ExpectedSyscallMatch
{
    ...
    ExpectedPoll(int fd, short events, int timeout, short revents, ssize_t res,
                 int errno_val = 0)
        : ExpectedSyscallMatch("poll"), fd(fd), events(events),
          timeout(timeout), revents(revents), res(res), errno_val(errno_val)
    {
    }
    ...
    int on_poll(struct pollfd* fds, nfds_t nfds, int timeout) override
    {
        ...
        if (!current)
            wfail_test(
                "expected file descriptor not found in input struct pollfd's");
...
struct ExpectedSendfile : public ExpectedSyscallMatch
{
    ...
    ExpectedSendfile(int out_fd, int in_fd, off_t offset, size_t count,
                     off_t roffset, ssize_t res, int errno_val = 0)
    ...
    ssize_t on_sendfile(int out_fd, int in_fd, off_t* offset,
...
struct ExpectedSplice : public ExpectedSyscallMatch
{
    ...
    ExpectedSplice(int fd_in, int fd_out, size_t len, unsigned int flags,
                   ssize_t res, int errno_val = 0)
    ...
    ssize_t on_splice(int fd_in, loff_t* off_in, int fd_out, loff_t* off_out,
                      size_t len, unsigned int flags) override
    {
        ARKI_UTILS_TEST_INFO(info);
        info << tag();
        using arki::utils::tests::actual;
