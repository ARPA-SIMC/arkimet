...
std::string StreamTestsFixture::streamed_contents()
{
    throw std::runtime_error(
        "streamed_contents is not available in this type of fixture");
}
...
stream::SendResult
StreamTestsFixture::send_file_segment(arki::core::NamedFileDescriptor& fd,
                                      off_t offset, size_t size)
{
    return output->send_file_segment(fd, offset, size);
}

stream::SendResult StreamTestsFixture::send_buffer(const void* data,
                                                   size_t size)
{
...
MockConcreteSyscalls::MockConcreteSyscalls()
    : orig_read(TestingBackend::read), orig_write(TestingBackend::write),
      orig_writev(TestingBackend::writev),
      orig_sendfile(TestingBackend::sendfile),
      orig_splice(TestingBackend::splice), orig_poll(TestingBackend::poll),
...
{
    TestingBackend::sendfile = [](int out_fd, int in_fd, off_t* offset,
                                  size_t count) noexcept -> ssize_t {
        errno = EINVAL;
        return -1;
    };
    TestingBackend::splice = [](int fd_in, loff_t* off_in, int fd_out,
                                loff_t* off_out, size_t len,
                                unsigned int flags) noexcept -> ssize_t {
        errno = EINVAL;
        return -1;
    };
}

ExpectedSyscalls::ExpectedSyscalls(std::vector<ExpectedSyscallMatch*> expected)
    : expected(expected)
{
    TestingBackend::read = [this](int fd, void* buf, size_t count) {
        return on_read(fd, buf, count);
    };
    TestingBackend::write = [this](int fd, const void* buf, size_t count) {
        return on_write(fd, buf, count);
    };
    TestingBackend::writev = [this](int fd, const struct iovec* iov,
                                    int iovcnt) {
        return on_writev(fd, iov, iovcnt);
    };
    TestingBackend::sendfile = [this](int out_fd, int in_fd, off_t* offset,
                                      size_t count) {
        return on_sendfile(out_fd, in_fd, offset, count);
    };
    TestingBackend::splice = [this](int fd_in, loff_t* off_in, int fd_out,
                                    loff_t* off_out, size_t len,
                                    unsigned int flags) {
        return on_splice(fd_in, off_in, fd_out, off_out, len, flags);
    };
    TestingBackend::poll = [this](struct pollfd* fds, nfds_t nfds,
                                  int timeout) {
        return on_poll(fds, nfds, timeout);
    };
    TestingBackend::pread = [this](int fd, void* buf, size_t count,
                                   off_t offset) {
        return on_pread(fd, buf, count, offset);
    };
...
{
    if (!expected.empty())
        wfail_test("function unexpectedly stopped before syscall " +
                   expected.front()->tag());
}
...
ssize_t ExpectedSyscalls::on_sendfile(int out_fd, int in_fd, off_t* offset,
                                      size_t count)
{
...
ssize_t ExpectedSyscalls::on_pread(int fd, void* buf, size_t count,
                                   off_t offset)
{
...
public:
    PipeSource(const std::string& data) : data(data)
    {
...
public:
    NonblockingPipeSource()
    {
        ...
        if (fl < 0)
            throw std::system_error(errno, std::system_category(),
                                    "cannot get file descriptor flags for read "
                                    "end of NonblockingPipeSource stdout pipe");
        if (fcntl(get_stdout(), F_SETFL, fl | O_NONBLOCK) < 0)
            throw std::system_error(
                errno, std::system_category(),
                "cannot set nonblocking file descriptor flags for "
                "NonblockingPipeSource stdout pipe");
    }
    ~NonblockingPipeSource()
    {
        if (!terminated())
    ...
    {
        sys::NamedFileDescriptor cmdfd(get_stdin(),
                                       "NonblockingPipeSource command pipe");
    ...
    void wait_for_data()
    {
        ...
        if (!(pollinfo.revents & POLLIN))
            throw std::runtime_error(
                "unsupported revents values on NonblockingPipeSource");
    }
};
...
void StreamTests::register_tests()
{
    ...
    add_method("send_line_filtered", [&] {
...

        {
            ...
            wassert(actual(f->send_line("testline1", 9)) ==
                    stream::SendResult());
    ...
    add_method("send_buffer_filtered", [&] {
...

        {
            ...
            wassert(actual(f->send_buffer("testbuf", 4)) ==
                    stream::SendResult());
            auto flt = f->stream().stop_filter();
    ...
    add_method("send_file_segment", [&] {
        ...
        wassert(actual(f->send_file_segment(tf1, 1, 6)) ==
                stream::SendResult());
        wassert(actual(f->send_file_segment(tf1, 5, 1)) ==
                stream::SendResult());
        wassert(actual(f->send_file_segment(tf1, 0, 4)) ==
                stream::SendResult());
    ...
    add_method("large_send_line", [&] {
        ...
        wassert(actual(f->send_line(buf.data(), buf.size())) ==
                stream::SendResult());
    ...
    add_method("large_send_buffer", [&] {
        ...
        wassert(actual(f->send_buffer(buf.data(), buf.size())) ==
                stream::SendResult());
    ...
    add_method("large_send_file_segment", [&] {
        ...
        wassert(actual(f->send_file_segment(tf1, 0, buf.size())) ==
                stream::SendResult());
