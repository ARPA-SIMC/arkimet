...
struct CommonTestFixture : public stream::StreamTestsFixture
{
    ...
    CommonTestFixture() : tf(std::make_shared<sys::Tempfile>())
    {
...
struct ConcreteTestFixture : public stream::StreamTestsFixture
{
    ...
    ConcreteTestFixture(std::shared_ptr<core::NamedFileDescriptor> out)
        : out(out)
    {
        set_output(std::unique_ptr<arki::StreamOutput>(
            new stream::ConcreteStreamOutputBase<stream::TestingBackend>(
                out, get_timeout_ms())));
    }
    ...
    ClosedPipe()
    {
        if (pipe(pipefds) < 0)
            throw std::system_error(errno, std::system_category(),
                                    "cannot create new pipe");
        // Close read end of the pipe
        close(pipefds[0]);

        fd = std::make_shared<sys::NamedFileDescriptor>(pipefds[1],
                                                        "write end of pipe");
    }
};

template <typename Fixture> class Tests : public stream::StreamTests
{
...
Tests<ConcreteTimeoutFallbackTestFixture>
    test4("arki_stream_concrete_timeout_fallback");

template <typename Fixture> void Tests<Fixture>::register_tests()
{
    ...
    add_method("closed_pipe_send_buffer", [&] {
        ...
        wassert(actual(f->send_buffer("A", 1)) ==
                stream::SendResult(stream::SendResult::SEND_PIPE_EOF_DEST));
    });

    add_method("closed_pipe_send_line", [&] {
        ClosedPipe cp;
        auto f = make_concrete_fixture(cp.fd);
        wassert(actual(f->send_line("A", 1)) ==
                stream::SendResult(stream::SendResult::SEND_PIPE_EOF_DEST));
    });
    ...
    add_method("closed_pipe_send_file_segment", [&] {
        ...
        wassert(actual(f->send_file_segment(tf, 1, 1)) ==
                stream::SendResult(stream::SendResult::SEND_PIPE_EOF_DEST));
    });
    ...
    add_method("read_eof", [&] {
        struct ReadEof : stream::DisableSendfileSplice
        {
            ...
            ReadEof(size_t lead_size = 0) : available(lead_size)
            {
                stream::TestingBackend::read =
                    [this](int fd, void* buf,
                ...
                stream::TestingBackend::pread =
                    [this](int fd, void* buf, size_t count,
        ...
        {
            ReadEof reof(10);
            auto f = make_concrete_fixture(
                std::make_shared<sys::File>("/dev/null", O_WRONLY));
            sys::NamedFileDescriptor input(0, "mock input");
            wassert(
                actual(f->send_file_segment(input, 5, 15)) ==
                stream::SendResult(stream::SendResult::SEND_PIPE_EOF_SOURCE));
        }
    });
    ...
    add_method("syscalls_buffer", [this] {
        ...
        // No timeout
        {
            stream::ExpectedSyscalls expected({
                new stream::ExpectedPoll(*outfile, POLLOUT,
                                         Fixture::get_timeout_ms(), POLLOUT, 1),
...

        {
            stream::ExpectedSyscalls expected({
                ...
                new stream::ExpectedPoll(*outfile, POLLOUT,
                                         Fixture::get_timeout_ms(), POLLERR, 1),
            });
            wassert(actual(writer->send_buffer("1234", 4)) ==
                    stream::SendResult(stream::SendResult::SEND_PIPE_EOF_DEST));
            wassert(expected.check_not_called());
...

        {
            stream::ExpectedSyscalls expected({
                ...
                new stream::ExpectedWrite(*outfile, "1234", 2),
                new stream::ExpectedPoll(*outfile, POLLOUT,
                                         Fixture::get_timeout_ms(), POLLHUP, 1),
            });
            wassert(actual(writer->send_buffer("1234", 4)) ==
                    stream::SendResult(stream::SendResult::SEND_PIPE_EOF_DEST));
            wassert(expected.check_not_called());
    ...
    add_method("syscalls_buffer_filtered", [this] {
        ...
        {
            stream::ExpectedSyscalls expected({
                new stream::ExpectedPoll(filter->cmd.get_stdin(), POLLOUT,
                                         Fixture::get_timeout_ms(), POLLOUT, 1),
        ...
        {
            stream::ExpectedSyscalls expected({
                new stream::ExpectedPoll(filter->cmd.get_stdout(), POLLIN,
                                         Fixture::get_timeout_ms(), POLLIN, 1),
                new stream::ExpectedPoll(*outfile, POLLOUT,
                                         Fixture::get_timeout_ms(), POLLOUT, 1),
                new stream::ExpectedSplice(
                    filter->cmd.get_stdout(), *outfile, 131072,
                    SPLICE_F_MORE | SPLICE_F_NONBLOCK, 4),
                new stream::ExpectedPoll(filter->cmd.get_stdout(), POLLIN,
                                         Fixture::get_timeout_ms(), POLLHUP, 1),
                new stream::ExpectedPoll(filter->cmd.get_stderr(), POLLIN,
                                         Fixture::get_timeout_ms(), POLLIN, 1),
    ...
    add_method("syscalls_buffer_filtered_readwrite", [this] {
...

        {
            stream::ExpectedSyscalls expected({
                ...
                new stream::ExpectedSplice(
                    filter->cmd.get_stdout(), *outfile, 131072,
                    SPLICE_F_MORE | SPLICE_F_NONBLOCK, -1, EINVAL),
                new stream::ExpectedPoll(filter->cmd.get_stdout(), POLLIN,
                                         Fixture::get_timeout_ms(), POLLIN, 1),
    ...
    add_method("syscalls_buffer_filtered_readwrite_regression2", [this] {
        arki::tests::skip_unless_splice();
        auto outfile = std::make_shared<sys::File>("/dev/null", O_WRONLY);
        auto writer  = make_concrete_fixture(outfile);
        auto filter  = writer->stream().start_filter({"cat"});
...

        {
            stream::ExpectedSyscalls expected({
                ...
                new stream::ExpectedPoll(filter->cmd.get_stderr(), POLLIN,
                                         Fixture::get_timeout_ms(), POLLHUP, 1),
                new stream::ExpectedPoll(filter->cmd.get_stdout(), POLLIN,
                                         Fixture::get_timeout_ms(),
                                         POLLIN | POLLHUP, 1),
                new stream::ExpectedRead(filter->cmd.get_stdout(), "12", 32768,
                                         2),
                new stream::ExpectedPoll(*outfile, POLLOUT,
                                         Fixture::get_timeout_ms(), POLLOUT, 1),
                new stream::ExpectedWrite(*outfile, "12", 2, 2),
                new stream::ExpectedPoll(filter->cmd.get_stdout(), POLLIN,
                                         Fixture::get_timeout_ms(),
                                         POLLIN | POLLHUP, 1),
                new stream::ExpectedRead(filter->cmd.get_stdout(), "34", 32768,
                                         2),
                new stream::ExpectedPoll(*outfile, POLLOUT,
                                         Fixture::get_timeout_ms(), POLLOUT, 1),
                new stream::ExpectedWrite(*outfile, "34", 2, 2),
                new stream::ExpectedPoll(filter->cmd.get_stdout(), POLLIN,
                                         Fixture::get_timeout_ms(), POLLHUP, 1),
            });
            auto flt = wcallchecked(writer->stream().stop_filter());
    ...
    add_method("syscalls_line_filtered", [this] {
...

        {
            stream::ExpectedSyscalls expected({
                ...
                new stream::ExpectedSplice(
                    filter->cmd.get_stdout(), *outfile, 131072,
                    SPLICE_F_MORE | SPLICE_F_NONBLOCK, 5),
                new stream::ExpectedPoll(filter->cmd.get_stdout(), POLLIN,
                                         Fixture::get_timeout_ms(), POLLHUP, 1),
                new stream::ExpectedPoll(filter->cmd.get_stderr(), POLLIN,
                                         Fixture::get_timeout_ms(), POLLIN, 1),
    ...
    add_method("syscalls_file", [this] {
        ...
        // Timeout
        {
            ...
            wassert_throws(stream::TimedOut,
                           writer->send_file_segment(tf, 1, 4));
            wassert(expected.check_not_called());
        }
        //    {
        //        sys::Tempfile tf1;
