...
bool BaseStreamOutput::is_nonblocking(int fd)
{
    ...
    if (src_fl < 0)
        throw std::system_error(errno, std::system_category(),
                                "cannot get file descriptor flags for input");
    return src_fl & O_NONBLOCK;
}

stream::FilterProcess*
BaseStreamOutput::start_filter(const std::vector<std::string>& command)
{
    if (filter_process)
        throw std::runtime_error(
            "A filter command was already started on this stream");
...
ssize_t (*LinuxBackend::writev)(int fd, const struct iovec* iov,
                                int iovcnt)         = ::writev;
ssize_t (*LinuxBackend::sendfile)(int out_fd, int in_fd, off_t* offset,
                                  size_t count)     = ::sendfile;
ssize_t (*LinuxBackend::splice)(int fd_in, loff_t* off_in, int fd_out,
                                loff_t* off_out, size_t len,
                                unsigned int flags) = ::splice;
int (*LinuxBackend::poll)(struct pollfd* fds, nfds_t nfds,
                          int timeout)              = ::poll;
ssize_t (*LinuxBackend::pread)(int fd, void* buf, size_t count,
                               off_t offset)        = ::pread;

std::function<ssize_t(int fd, void* buf, size_t count)> TestingBackend::read =
    ::read;
std::function<ssize_t(int fd, const void* buf, size_t count)>
...
std::function<ssize_t(int fd_in, loff_t* off_in, int fd_out, loff_t* off_out,
                      size_t len, unsigned int flags)>
    TestingBackend::splice = ::splice;
std::function<int(struct pollfd* fds, nfds_t nfds, int timeout)>
