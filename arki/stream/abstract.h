...
struct AbstractStreamOutput : public BaseStreamOutput
{
     ...
     */
    virtual stream::SendResult _write_output_buffer(const void* data,
                                                    size_t size) = 0;
     ...
     */
    virtual stream::SendResult _write_output_line(const void* data,
                                                  size_t size);
    ...
    // Generic implementation based on _write_output_buffer
    SendResult send_file_segment(arki::core::NamedFileDescriptor& fd,
                                 off_t offset, size_t size) override;
};
