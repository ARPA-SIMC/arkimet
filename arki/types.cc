...
Code checkCodeName(const std::string& name)
{
    ...
    if (nname == "source")
        return TYPE_SOURCE;
    if (nname == "assigneddataset")
        return TYPE_ASSIGNEDDATASET;
    if (nname == "area")
        return TYPE_AREA;
    if (nname == "proddef")
        return TYPE_PRODDEF;
    if (nname == "summaryitem")
        return TYPE_SUMMARYITEM;
    if (nname == "summarystats")
        return TYPE_SUMMARYSTATS;
    if (nname == "bbox")
        return TYPE_BBOX;
    if (nname == "run")
        return TYPE_RUN;
...
int Type::compare(const Type& o) const { return type_code() - o.type_code(); }
...
void Type::serialise(structured::Emitter& e, const structured::Keys& keys,
                     const Formatter* f) const
{
    e.start_mapping();
    e.add(keys.type_name, tag());
    if (f)
        e.add(keys.type_desc, f->format(*this));
...
unique_ptr<Type> Type::decodeInner(types::Code code, core::BinaryDecoder& dec)
{
    return std::unique_ptr<Type>(
        types::MetadataType::get(code)->decode_func(dec, false));
}

unique_ptr<Type> Type::decode_inner(types::Code code, core::BinaryDecoder& dec,
                                    bool reuse_buffer)
{
    return std::unique_ptr<Type>(
        types::MetadataType::get(code)->decode_func(dec, reuse_buffer));
}

unique_ptr<Type> decodeString(types::Code code, const std::string& val)
{
    return std::unique_ptr<Type>(
        types::MetadataType::get(code)->string_decode_func(val));
}
...
{
    return std::unique_ptr<Type>(
        types::MetadataType::get(code)->structure_decode_func(keys, reader));
}
