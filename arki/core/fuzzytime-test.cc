...
static void parses(int ye, int mo = -1, int da = -1, int ho = -1, int mi = -1,
                   int se = -1)
{
    ...
    {
        std::stringstream ss;
        ss << t.to_string() << " did not validate: " << e.what();
        throw TestFailed(ss.str());
    }
}

static void fails(const char* what, int ye, int mo = -1, int da = -1,
                  int ho = -1, int mi = -1, int se = -1)
{
    FuzzyTime t(ye, mo, da, ho, mi, se);
...
void Tests::register_tests()
{
    ...
    add_method("validate", [] {
        ...
        wassert(
            fails("minute must be between 0 and 59", 2024, 2, 1, 23, 60, 0));
        wassert(parses(2024, 2, 1, 23, 59, 60));
        wassert(
            fails("second must be between 0 and 60", 2024, 2, 1, 23, 59, 61));
    });
}
