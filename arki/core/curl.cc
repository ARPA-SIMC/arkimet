...
Exception::Exception(CURLcode code, const std::string& extrainfo,
...
CurlEasy::CurlEasy()
{
    ...
    if (!m_curl)
        throw std::runtime_error(
            "cannot initialize CURL: curl_easy_init returned NULL");
}
...
{
    // See
    // file:///usr/share/doc/libcurl4-gnutls-dev/html/libcurl/curl_easy_setopt.html
    curl_easy_reset(m_curl);
    checked("setting error buffer",
            curl_easy_setopt(m_curl, CURLOPT_ERRORBUFFER, m_errbuf));
    // checked("setting verbose", curl_easy_setopt(m_curl, CURLOPT_VERBOSE, 1));
    checked("setting user agent",
            curl_easy_setopt(m_curl, CURLOPT_USERAGENT, "arkimet"));
    // CURLOPT_PROGRESSFUNCTION / CURLOPT_PROGRESSDATA ?
}

CurlForm::CurlForm(CurlEasy& curl) { post = curl_mime_init(curl); }

CurlForm::~CurlForm() { clear(); }
...
Request::Request(CurlEasy& curl) : curl(curl), method("GET"), post_data(curl) {}
...
void Request::perform()
{
    ...
    while (true)
    {
        ...
        if (method == "POST")
        {
            checked("selecting POST method",
                    curl_easy_setopt(curl, CURLOPT_POST, 1));
            checked("setting POST data",
                    curl_easy_setopt(curl, CURLOPT_MIMEPOST, post_data.get()));
        }
        else if (method == "GET")
            checked("selecting GET method",
                    curl_easy_setopt(curl, CURLOPT_HTTPGET, 1));
        else
            throw std::runtime_error("requested unsupported HTTP method '" +
                                     method + "'");
        checked("setting HTTP authentication method",
                curl_easy_setopt(curl, CURLOPT_HTTPAUTH, CURLAUTH_ANY));
        checked("setting netrc usage",
                curl_easy_setopt(curl, CURLOPT_NETRC, CURL_NETRC_OPTIONAL));
        checked("setting header function",
                curl_easy_setopt(curl, CURLOPT_HEADERFUNCTION,
                                 &Request::headerfunc));
        checked("setting header function data",
                curl_easy_setopt(curl, CURLOPT_WRITEHEADER, this));
        checked(
            "setting write function",
            curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, &Request::writefunc));
        checked("setting write function data",
        ...
        if (code != CURLE_OK)
        {
            ...
            throw core::curl::Exception(code, curl.m_errbuf,
                                        "Cannot query " + actual_url);
        }

        if (response_code == -1)
            checked("reading response code",
        ...
        if (response_code >= 400)
        {
            ...
            msg << method << " " << actual_url << " got response code "
                << response_code << ": ";
        ...
        {
            char* url = nullptr;
            checked("reading redirect url",
                    curl_easy_getinfo(curl, CURLINFO_REDIRECT_URL, &url));
            if (url)
                actual_url = url;
            else
                throw std::runtime_error(
                    "redirect code " + std::to_string(response_code) +
                    " received without a redirect url available");
        }
        else if (response_code >= 200)
        {
            ...
            throw std::runtime_error("received unsupported HTTP code " +
...
size_t Request::writefunc(void* ptr, size_t size, size_t nmemb, void* stream)
{
    ...
    if (req.response_code == -1)
        checked("reading response code",
                curl_easy_getinfo(req.curl, CURLINFO_RESPONSE_CODE,
                                  &req.response_code));
