...
template <class Data> struct FixtureWriter : public DatasetTest
{
...
TestsWriter<GRIBData> test_writer_grib_simple("arki_dataset_writer_grib_simple",
                                              "type=simple\nformat=grib\n");
TestsWriter<GRIBData> test_writer_grib_iseg("arki_dataset_writer_grib_iseg",
                                            "type=iseg\nformat=grib\n");
TestsWriter<BUFRData> test_writer_bufr_simple("arki_dataset_writer_bufr_simple",
                                              "type=simple\nformat=bufr\n");
TestsWriter<BUFRData> test_writer_bufr_iseg("arki_dataset_writer_bufr_iseg",
                                            "type=iseg\nformat=bufr\n");
TestsWriter<VM2Data> test_writer_vm2_simple("arki_dataset_writer_vm2_simple",
                                            "type=simple\nformat=vm2\n");
TestsWriter<VM2Data> test_writer_vm2_iseg("arki_dataset_writer_vm2_iseg",
                                          "type=iseg\nformat=vm2\n");
TestsWriter<ODIMData> test_writer_odim_simple("arki_dataset_writer_odim_simple",
                                              "type=simple\nformat=odimh5\n");
TestsWriter<ODIMData> test_writer_odim_iseg("arki_dataset_writer_odim_iseg",
                                            "type=iseg\nformat=odimh5\n");
TestsWriter<NCData> test_writer_nc_simple("arki_dataset_writer_nc_simple",
                                          "type=simple\nformat=nc\n");
TestsWriter<NCData> test_writer_nc_iseg("arki_dataset_writer_nc_iseg",
                                        "type=iseg\nformat=nc\n");
TestsWriter<JPEGData> test_writer_jpeg_simple("arki_dataset_writer_jpeg_simple",
                                              "type=simple\nformat=nc\n");
TestsWriter<JPEGData> test_writer_jpeg_iseg("arki_dataset_writer_jpeg_iseg",
                                            "type=iseg\nformat=jpeg\n");
...
void Tests::register_tests()
{
    ...
    add_method("import_largefile", [](Fixture& f) {
        ...
        f.segment_session()->default_file_segment =
...

        {
            ...
            for (unsigned day = 1; day <= 30; ++day)
            {
                ...
                {
                    auto md = make_large_mock(DataFormat::GRIB,
                                              10 * 1024 * 1024, 12, day, hour);
    ...
    add_method("import_batch_replace_usn", [](Fixture& f) {
        ...
        wassert(actual_type(mdc[0].source())
                    .is_source_blob(DataFormat::BUFR, f.ds_root,
                                    "2009/12-04.bufr"));

        // Acquire again: it works, since USNs the same as the existing ones do
        // overwrite
        wassert(ds->acquire_batch(batch, ReplaceStrategy::HIGHER_USN));
        wassert(actual(batch[0]->result) == metadata::Inbound::Result::OK);
        wassert(actual(batch[0]->destination) == "testds");
        ...
        {
            wassert(actual(batch[0]->result) ==
                    metadata::Inbound::Result::DUPLICATE);
    ...
    add_method("issue237", [](Fixture& f) {
        ...
        // Acquire value
        {
            ...
            wassert(actual(batch[0]->result) == metadata::Inbound::Result::OK);
            wassert(actual_type(batch[0]->md->source())
                        .is_source_blob(DataFormat::VM2, f.ds_root,
                                        "2020/10-31.vm2", 0, 36));
        }
        ...
        // Read it back
        {
            ...
            wassert(actual_type(mdc1[0].source())
                        .is_source_blob(DataFormat::VM2, f.ds_root,
                                        "2020/10-31.vm2", 0, 36));
            auto data = mdc1[0].get_data().read();
            wassert(actual(std::string(
                        reinterpret_cast<const char*>(data.data()),
                        data.size())) == "202010312300,12865,158,9.409990,,,");
        }

        wassert(actual_file(f.ds_root / "2020/10-31.vm2")
                    .contents_equal("20201031230000,12865,158,9.409990,,,\n"));

        auto state = f.scan_state();
        wassert(actual(state.size()) == 1u);
        wassert(actual(state.get("testds:2020/10-31.vm2").state) ==
                segment::SEGMENT_OK);
    });
}

template <class Data> void TestsWriter<Data>::register_tests()
{
    ...
    this->add_method("import", [](Fixture& f) {
        ...
        for (unsigned i = 0; i < 3; ++i)
        {
            ...
            wassert(actual_type(md->source())
                        .is_source_blob(f.td.format, f.ds_root,
                                        f.destfile(f.td.mds[i])));
        }
    });
    ...
    this->add_method("import_error", [](Fixture& f) {
        ...
        md.set_source_inline(
            format, metadata::DataManager::get().to_unreadable_data(1));
        ...
        auto state = f.scan_state();
        wassert(actual(state.size()) == 1u);
        wassert(actual(state.get("testds:2018/01-01." + format_name(format))
    ...
    this->add_method("import_batch_replace_never", [](Fixture& f) {
        ...
        for (unsigned i = 0; i < 3; ++i)
        {
            ...
            wassert(actual_file(f.ds_root / f.destfile(f.td.mds[i])).exists());
            wassert(actual_type(f.td.mds[i].source())
                        .is_source_blob(f.td.format, f.ds_root,
                                        f.destfile(f.td.mds[i])));
        }
        ...
        for (unsigned i = 0; i < 3; ++i)
        {
            if (ds->type() == "simple")
            {
                wassert(actual(batch[i]->result) ==
                        metadata::Inbound::Result::OK);
                wassert(actual(batch[i]->destination) == "testds");
                wassert(actual(mds[i].sourceBlob().absolutePathname()) ==
                        f.td.mds[i].sourceBlob().absolutePathname());
                wassert(actual(mds[i].sourceBlob().offset) >
                        f.td.mds[i].sourceBlob().offset);
                wassert(actual(mds[i].sourceBlob().size) ==
                        f.td.mds[i].sourceBlob().size);
            }
            else
            {
                wassert(actual(batch[i]->result) ==
    ...
    this->add_method("import_before_archive_age", [](Fixture& f) {
        auto o = dataset::SessionTime::local_override(
            1483225200); // date +%s --date="2017-01-01"
    ...
    auto test_same_segment_fail = [](Fixture& f, unsigned fail_idx,
        ...
        {
            md.test_set(
                types::Reftime::createPosition(Time(2018, 1, 1, idx, 0, 0)));
            if (idx == fail_idx)
                md.set_source_inline(
                    format, metadata::DataManager::get().to_unreadable_data(1));
            else
                md.set_source_inline(
                    ...
                    metadata::DataManager::get().to_data(
                        format, std::vector<uint8_t>{(unsigned char)idx}));
            mds.push_back(md);
        }
    ...
    auto test_different_segment_fail = [](Fixture& f, unsigned fail_idx,
        ...
        for (unsigned idx = 0; idx < 3; ++idx)
        {
            md.test_set(types::Reftime::createPosition(
                Time(2018, idx + 1, 1, 0, 0, 0)));
            if (idx == fail_idx)
                md.set_source_inline(
                    format, metadata::DataManager::get().to_unreadable_data(1));
            else
        ...
        wassert(ds->acquire_batch(batch, strategy));
        wassert(actual(batch[0]->result) ==
                (fail_idx == 0 ? metadata::Inbound::Result::ERROR
                               : metadata::Inbound::Result::OK));
        wassert(actual(batch[1]->result) ==
                (fail_idx == 1 ? metadata::Inbound::Result::ERROR
                               : metadata::Inbound::Result::OK));
        wassert(actual(batch[2]->result) ==
                (fail_idx == 2 ? metadata::Inbound::Result::ERROR
                               : metadata::Inbound::Result::OK));
        ds->flush();
        ...
        wassert(
            actual(
                state.get("testds:2018/01-01." + format_name(format)).state) ==
            (fail_idx == 0 ? segment::SEGMENT_DELETED : segment::SEGMENT_OK));
        wassert(
            actual(
                state.get("testds:2018/02-01." + format_name(format)).state) ==
            (fail_idx == 1 ? segment::SEGMENT_DELETED : segment::SEGMENT_OK));
        wassert(
            actual(
                state.get("testds:2018/03-01." + format_name(format)).state) ==
            (fail_idx == 2 ? segment::SEGMENT_DELETED : segment::SEGMENT_OK));
    ...
    this->add_method(
        "transaction_different_segment_fail_first", [=](Fixture& f) {
            ...
            wassert(
                test_different_segment_fail(f, 0, ReplaceStrategy::HIGHER_USN));
        });

    this->add_method(
        "transaction_different_segment_fail_middle", [=](Fixture& f) {
            ...
            wassert(
                test_different_segment_fail(f, 1, ReplaceStrategy::HIGHER_USN));
        });

    this->add_method(
        "transaction_different_segment_fail_last", [=](Fixture& f) {
            ...
            wassert(
                test_different_segment_fail(f, 2, ReplaceStrategy::HIGHER_USN));
        });
