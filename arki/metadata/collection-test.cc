...
add_method("without_duplicates_empty_collection", []() {
    Collection mds;
    Collection mds1 = mds.without_duplicates(types::parse_code_names("product, level"));
...
add_method("without_duplicates_no_unique_metadata_items", []() {
    GRIBData td;
    Collection mds = td.mds.without_duplicates(types::parse_code_names("task, quantity"));
    wassert_true(mds.empty());
});

add_method("without_duplicates_no_duplicates", []() {
    GRIBData td;
    Collection mds = td.mds.without_duplicates(types::parse_code_names("product, level"));
    wassert_true(mds == td.mds);
});
...
add_method("without_duplicates_all_duplicates", []() {
    ...
    auto mds1 = mds.without_duplicates(types::parse_code_names("product, level"));
    wassert(actual(mds1.size()) == 1u);
    wassert(actual_type(mds1[0].get(TYPE_REFTIME)).is_reftime_position(core::Time(2025, 1, 3)));
});
...
add_method("without_duplicates", []() {
    ...
    wassert(actual_type(mds1[0].get(TYPE_REFTIME)).is_reftime_position(core::Time(2025, 1, 4)));
    wassert(actual_type(mds1[1].get(TYPE_REFTIME)).is_reftime_position(core::Time(2025, 1, 5)));
    wassert(actual_type(mds1[2].get(TYPE_REFTIME)).is_reftime_position(core::Time(2025, 1, 7)));
});
