...
bool FailAlways::operator()(const Metadata& v,
...
{
    name = "daily_import";
    desc = "Checks that the reference time is less than a week old and not "
           "more than a day into the future";
}
bool DailyImport::operator()(const Metadata& v,
                             std::vector<std::string>& errors) const
{
    ...
    // today[3], today[4], today[5]); printf("VAL   %s\n",
    // rt->time.toSQL().c_str()); printf("SECS %d\n", secs);
    if (secs > 3600 * 24 * 7)
    {
    ...
    {
        errors.push_back(
            name + ": reference time is more than one day into the future");
        return false;
    }
