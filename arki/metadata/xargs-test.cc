...
void Tests::register_tests()
{
    ...
    add_method("check_env", [] {
        ...
        wassert(actual(out).contains(
            "ARKI_XARGS_TIME_START='2007-07-08 13:00:00Z'\n"));
        wassert(actual(out).contains(
            "ARKI_XARGS_TIME_END='2007-07-08 13:00:00Z'\n"));
    });
    ...
    add_method("interval", [] {
        ...
        string out = sys::read_file("tmp-xargs");
        wassert(actual(out).matches(
            "([[:alnum:]/]+/arki-xargs.[[:alnum:]]+\n){3}"));
    });

    add_method("issue124", [] {
        sys::rmtree_ifexists("xargs_tmpdir");
        std::filesystem::create_directory("xargs_tmpdir");
        sys::OverrideEnvironment oe("TMPDIR",
                                    std::filesystem::canonical("xargs_tmpdir"));
