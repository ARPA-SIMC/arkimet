...
class BaseTests : public TestCase
{
    ...
    virtual std::unique_ptr<metadata::ArchiveOutput>
    create_archive(const std::string& format, const std::string& fname) = 0;
...
class TestFile : public BaseTests
{
    ...
    std::unique_ptr<metadata::ArchiveOutput>
    create_archive(const std::string& format, const std::string& fname) override
    {
        return metadata::ArchiveOutput::create_file(
            format,
            std::make_shared<sys::File>(fname, O_WRONLY | O_CREAT | O_TRUNC));
    }
...
class TestStremOutput : public BaseTests
{
    ...
    create_archive(const std::string& format, const std::string& fname) override
    {
        return metadata::ArchiveOutput::create_stream(
            format, StreamOutput::create(std::make_shared<sys::File>(
                        fname, O_WRONLY | O_CREAT | O_TRUNC)));
    }
...
void BaseTests::register_tests()
{
    add_method("tar", [&] {
        ...
        wassert(actual(sys::read_file("test.out")) == "data/000001.grib\n"
                                                      "data/000002.grib\n"
                                                      "data/000003.grib\n"
                                                      "data/metadata.md\n");
    });
...
---------  ---------- -----   ----
    ...
    34960  (2007-07-07|07-07-2007) [0-9]+:00   data/000002.grib
     2234  (2007-10-09|10-09-2007) [0-9]+:00   data/000003.grib
      442  [0-9]+-[0-9]+-[0-9]+ [0-9]+:[0-9]+   data/metadata.md
---------                     -------
    44854                     4 files
)"));
    });
