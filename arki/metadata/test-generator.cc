...
Generator::~Generator()
{
    for (Samples::iterator i = samples.begin(); i != samples.end(); ++i)
        for (std::vector<types::Type*>::iterator j = i->second.begin();
             j != i->second.end(); ++j)
            delete *j;
}
...
void Generator::defaults_grib2()
{
    ...
    add_if_missing(TYPE_REFTIME, "2010-06-01T00:00:00Z");
    add_if_missing(TYPE_AREA,
                   "GRIB(Ni=511, Nj=415, latfirst=-16125000, latlast=9750000, "
                   "latp=-40000000, lonfirst=344250000, lonlast=16125000, "
                   "lonp=10000000, rot=0, tn=1)");
...
void Generator::defaults_bufr()
{
    ...
    add_if_missing(TYPE_REFTIME, "2010-06-01T00:00:00Z");
    add_if_missing(TYPE_AREA,
                   "GRIB(blo=60, lat=3388000, lon=-553000, sta=150)");
}
...
void Generator::defaults_odimh5()
{
    ...
    add_if_missing(TYPE_REFTIME, "2010-06-01T00:00:00Z");
    add_if_missing(TYPE_AREA,
                   "ODIMH5(lat=44456700, lon=11623600, radius=1000)");
    add_if_missing(TYPE_TASK, "task");
    add_if_missing(
        TYPE_QUANTITY,
        "ACRR, BRDR, CLASS, DBZH, DBZV, HGHT, KDP, LDR, PHIDP, QIND, RATE, "
        "RHOHV, SNR, SQI, TH, TV, UWND, VIL, VRAD, VWND, WRAD, ZDR, ad, "
        "ad_dev, chi2, dbz, dbz_dev, dd, dd_dev, def, def_dev, div, div_dev, "
        "ff, ff_dev, n, rhohv, rhohv_dev, w, w_dev, z, z_dev");
}
    ...
    switch (format)
    {
        ...
        default:
            throw std::runtime_error(
                "cannot generate random messages: unknown format: " +
                format_name(format));
    }
...
bool Generator::_generate(const Samples::const_iterator& i, Metadata& md,
                          metadata_dest_func cons) const
{
    if (i == samples.end())
    {
        ...
        const types::reftime::Position* p =
            dynamic_cast<const types::reftime::Position*>(rt);
        auto t = p->get_Position();
        m->set(types::Run::createMinute(t.ho, t.mi));

        // Set source and inline data
        m->set_source_inline(format, metadata::DataManager::get().to_data(
                                         format, vector<uint8_t>(5432)));

        return cons(move(m));
    }

    for (vector<Type*>::const_iterator j = i->second.begin();
         j != i->second.end(); ++j)
    {
        md.test_set(**j);
        Samples::const_iterator next = i;
        ++next;
        if (!_generate(next, md, cons))
            return false;
    }
    return true;
}
