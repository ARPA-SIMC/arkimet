...
Xargs::Xargs() : metadata::Clusterer(), filename_argument(-1)
{
...
void Xargs::flush_batch()
{
    ...
    {
        stringstream ss;
        ss << "cannot run " << command[0] << ": process returned exit status "
           << res;
...
int Xargs::run_child()
{
    ...
    {
        string envstr(*s);
        if (str::startswith(envstr, "ARKI_XARGS_"))
            continue;
...
static size_t parse_interval(const std::string& str)
{
    ...
    throw std::runtime_error(
        "cannot parse interval name: unsupported interval: " + str +
        ".  Valid intervals are minute, hour, day, month and year");
}

void Xargs::set_max_bytes(size_t val) { max_bytes = val; }
