...
/**
 * Creates a tempfile, runs body and returns the contents of the temp file after
 * that.
 *
 ...
 */
std::string tempfile_to_string(
    std::function<void(arki::utils::sys::NamedFileDescriptor& out)> body)
{
...
inline bool cmpmd(Metadata& md1, Metadata& md2)
{
    if (md1 != md2)
    {
        ...
        if (md1.source().style() == types::Source::Style::INLINE)
        {
            ...
            cerr << "-- Inline data:"
                 << string((const char*)buf.data(), buf.size()) << endl;
        }
...
void Tests::register_tests()
{
    ...
    add_method("stream", [] {
        ...
        md1->set_source(types::Source::createURL(DataFormat::GRIB,
        ...
        md1->set_source_inline(
            DataFormat::GRIB,
            metadata::DataManager::get().to_data(
                DataFormat::GRIB, vector<uint8_t>(teststr, teststr + 14)));

        // Encode everything in a buffer
        size_t end1, end2;
        std::string input =
            tempfile_to_string([&](sys::NamedFileDescriptor& out) {
                md1->write(out);
    ...
    add_method("split", [] {
        // Create test metadata
        Metadata md;
        md.set_source(types::Source::createURL(DataFormat::GRIB,
                                               "http://www.example.org"));
        fill(md);
