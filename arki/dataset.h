 ...
 *
 * A dataset must be fully independent from other datasets, and not must not
 ...
 *
 * Since the dataset groups data with similar properties, the summary is
 * intended to change rarely.
 *
...
public:
    ...
    std::shared_ptr<const core::cfg::Section> config() const
    {
...
protected:
     ...
     */
    virtual bool impl_query_data(const query::Data& q,
     ...
     */
    virtual void impl_stream_query_bytes(const query::Bytes& q,
                                         StreamOutput& out);
...
public:
     ...
     *
     * @return The outcome of the operation, as a vector with an
     * WriterAcquireResult for each metadata in the collection.
     */
    virtual void acquire_batch(metadata::InboundBatch& batch,
                               const AcquireConfig& cfg = AcquireConfig()) = 0;
...
struct CheckerConfig
{
    ...
    CheckerConfig();
    CheckerConfig(std::shared_ptr<dataset::Reporter> reporter,
                  bool readonly = true);
