...
public:
    ...
    public:
        ...
        const_iterator(Tokenizer& tok) : tok(tok), beg(0), end(0)
        {
            operator++();
        }
        const_iterator(Tokenizer& tok, bool)
            : tok(tok), beg(tok.str.size()), end(tok.str.size())
        {
        }

        const_iterator& operator++();

        std::string operator*() const { return tok.str.substr(beg, end - beg); }
...
public:
    ...
    public:
        ...
        const_iterator(Regexp& re, const std::string& str) : re(re), next(str)
        {
            ++*this;
        }
     ...
     */
    Splitter(const std::string& re, int flags) : re(re, 1, flags) {}
