...
Proddef::Style Proddef::parseStyle(const std::string& str)
{
    if (str == "GRIB")
        return Style::GRIB;
    throw_consistency_error("parsing Proddef style",
                            "cannot parse Proddef style '" + str +
                                "': only GRIB is supported");
}
    ...
    switch (sty)
    {
        ...
        default:
            throw_consistency_error(
                "parsing Proddef", "unknown Proddef style " + formatStyle(sty));
    }
}
                                         ...
                                         bool reuse_buffer)
{
    ...
    switch (sty)
    {
        case Style::GRIB:
            ...
            break;
        default:
            throw std::runtime_error("cannot parse Proddef: style is " +
                                     formatStyle(sty) +
                                     " but we can only decode GRIB");
    }
    return res;
}
...
std::unique_ptr<Proddef>
Proddef::decode_structure(const structured::Keys& keys,
                          const structured::Reader& val)
{
    ...
    {
        case Style::GRIB:
            val.sub(keys.proddef_value, "proddef value",
...
void Proddef::init() { MetadataType::register_type<Proddef>(); }
