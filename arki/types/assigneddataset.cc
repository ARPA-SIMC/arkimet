...
int AssignedDataset::compare(const Type& o) const
{
    ...
    if (!v)
        throw_consistency_error(
            ...
            std::string(
                "second element claims to be a AssignedDataset, but it is a ") +
...
void AssignedDataset::get(core::Time& changed, std::string& name,
                          std::string& id) const
{
...
std::unique_ptr<AssignedDataset>
AssignedDataset::decode(core::BinaryDecoder& dec, bool reuse_buffer)
{
...
void AssignedDataset::serialise_local(structured::Emitter& e,
                                      ...
                                      const Formatter* f) const
{
    ...
    std::string name, id, vname, vid;
    get(changed, name, id);
    e.add(keys.assigneddataset_time);
    e.add(changed);
...
std::unique_ptr<AssignedDataset>
AssignedDataset::decode_structure(const structured::Keys& keys,
...
unique_ptr<AssignedDataset>
AssignedDataset::decodeString(const std::string& val)
{
    ...
    if (idpos == string::npos)
        throw_consistency_error(
            "parsing dataset attribution",
            "string \"" + val +
                "\" does not contain \" imported on \" after the dataset name");
...
std::unique_ptr<AssignedDataset>
AssignedDataset::create(const std::string& name, const std::string& id)
{
    return create(Time::create_now(), name, id);
}

std::unique_ptr<AssignedDataset>
AssignedDataset::create(const Time& changed, const std::string& name,
                        const std::string& id)
{
    ...
    enc.add_raw(name);
    enc.add_unsigned(id.size(), 2);
    enc.add_raw(id);
    return std::unique_ptr<AssignedDataset>(new AssignedDataset(buf));
}

void AssignedDataset::init() { MetadataType::register_type<AssignedDataset>(); }
