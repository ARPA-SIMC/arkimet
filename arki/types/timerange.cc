...
static bool GRIB1_get_timeunit_conversion(t_enum_GRIB_TIMEUNIT unit,
                                          int& timemul)
{
    ...
    switch (unit)
    {
        case GRIB_TIMEUNIT_UNKNOWN:
            throw_consistency_error("normalising TimeRange",
                                    "time unit is UNKNOWN (-1)");
        ...
        default:                    {
            std::stringstream ss;
            ss << "cannot normalise TimeRange: time unit is unknown ("
               << (unsigned)unit << ")";
...
static std::string formatTimeUnit(t_enum_GRIB_TIMEUNIT unit)
{
    ...
    {
        case GRIB_TIMEUNIT_UNKNOWN:
            throw_consistency_error("formatting TimeRange unit",
                                    "time unit is UNKNOWN (-1)");
...
{
    if (tu == "m")
        return GRIB_TIMEUNIT_MINUTE;
    if (tu == "h")
        return GRIB_TIMEUNIT_HOUR;
    if (tu == "d")
        return GRIB_TIMEUNIT_DAY;
    if (tu == "mo")
        return GRIB_TIMEUNIT_MONTH;
    if (tu == "y")
        return GRIB_TIMEUNIT_YEAR;
    if (tu == "de")
        return GRIB_TIMEUNIT_DECADE;
    if (tu == "no")
        return GRIB_TIMEUNIT_NORMAL;
    if (tu == "ce")
        return GRIB_TIMEUNIT_CENTURY;
    if (tu == "h3")
        return GRIB_TIMEUNIT_HOURS3;
    if (tu == "h6")
        return GRIB_TIMEUNIT_HOURS6;
    if (tu == "h12")
        return GRIB_TIMEUNIT_HOURS12;
    if (tu == "s")
        return GRIB_TIMEUNIT_SECOND;
    throw_consistency_error("parsing TimeRange unit",
                            "unknown time unit \"" + tu + "\"");
}
...
Timerange::Style Timerange::parseStyle(const std::string& str)
{
    ...
    if (str == "GRIB2")
        return Style::GRIB2;
    if (str == "Timedef")
        return Style::TIMEDEF;
    if (str == "BUFR")
        return Style::BUFR;
    throw_consistency_error(
        "parsing Timerange style",
        "cannot parse Timerange style '" + str +
            "': only GRIB1, GRIB2, Timedef and BUFR are supported");
}
...
int Timerange::compare(const Type& o) const
{
    ...
    switch (sty)
    {
        case timerange::Style::GRIB1:
            return reinterpret_cast<const timerange::GRIB1*>(this)
                ->compare_local(*reinterpret_cast<const timerange::GRIB1*>(v));
        case timerange::Style::GRIB2:
            return reinterpret_cast<const timerange::GRIB2*>(this)
                ->compare_local(*reinterpret_cast<const timerange::GRIB2*>(v));
        case timerange::Style::TIMEDEF:
            return reinterpret_cast<const timerange::Timedef*>(this)
                ->compare_local(
                    *reinterpret_cast<const timerange::Timedef*>(v));
        case timerange::Style::BUFR:
            return reinterpret_cast<const timerange::BUFR*>(this)
                ->compare_local(*reinterpret_cast<const timerange::BUFR*>(v));
        default:
...
void Timerange::get_GRIB1(const uint8_t* data, unsigned size, unsigned& type,
...
void Timerange::get_GRIB1_normalised(const uint8_t* data, unsigned size,
                                     int& otype, timerange::GRIB1Unit& ounit,
                                     int& op1, int& op2, bool& use_op1,
                                     bool& use_op2)
{
    ...
    int timemul;
    if (GRIB1_get_timeunit_conversion(static_cast<t_enum_GRIB_TIMEUNIT>(unit),
                                      timemul))
    ...
    switch ((t_enum_GRIB_TIMERANGE)type)
    {
        case GRIB_TIMERANGE_FORECAST_AT_REFTIME_PLUS_P1:
            ...
            use_op1 = true;
            break;
        case GRIB_TIMERANGE_ANALYSIS_AT_REFTIME: break;
...
void Timerange::get_GRIB2(const uint8_t* data, unsigned size, unsigned& type,
...
void Timerange::get_Timedef(const uint8_t* data, unsigned size,
...
{
    core::BinaryDecoder dec(data + 1, size - 1);
    step_unit = static_cast<timerange::TimedefUnit>(
        dec.pop_byte("timedef forecast step unit"));
    step_len = 0;
    if (step_unit != 255)
    ...
    if (stat_type != 255)
    {
        stat_unit = static_cast<timerange::TimedefUnit>(
            dec.pop_byte("timedef statistical processing unit"));
        if (stat_unit != 255)
            stat_len = dec.pop_varint<uint32_t>(
                "timedef statistical processing length");
    }
}

void Timerange::get_BUFR(const uint8_t* data, unsigned size, unsigned& unit,
                         unsigned& value)
{
    core::BinaryDecoder dec(data + 1, size - 1);
    ...
    switch (sty)
    {
        case timerange::Style::GRIB1: {
            ...
            if (endptr == start)
                throw_consistency_error(
                    ...
                    "expected type, but found \"" +
                        inner.substr(start - inner.c_str()) + "\"");
            start = endptr;
            ...
            if (*start)
            {
                ...
                if (*start)
                {
                    ...
                    {
                        p2tu =
                            inner.substr(start - inner.c_str(), sfxend - start);
                        start = sfxend;
                    }
                }
            }
            ...
            if (*start)
                throw_consistency_error(
                    "parsing TimeRange",
                    "found trailing characters at the end: \"" +
                        inner.substr(start - inner.c_str()) + "\"");
            ...
            switch ((t_enum_GRIB_TIMERANGE)type)
            {
                ...
                case GRIB_TIMERANGE_AVERAGE_OF_SUCCESSIVE_FORECAST_AVERAGES:
                    ...
                    if (p1tu != p2tu)
                        throw_consistency_error(
                            "parsing TimeRange",
                            "time unit for p1 (\"" + p1tu +
                                "\") is different than time unit of p2 (\"" +
                                p2tu + "\")");
        ...
        case timerange::Style::TIMEDEF: {
            ...
            else
            {
                ...
                if (*str && !timerange::Timedef::timeunit_parse(str, stat_unit,
                                                                stat_len))
                    throw_consistency_error(
                        "parsing Timerange",
                        "cannot parse length of statistical processing");
            }

            return createTimedef(step_len, step_unit, stat_type, stat_len,
                                 stat_unit);
        }
        default:
            throw_consistency_error("parsing Timerange",
...
std::unique_ptr<Timerange>
Timerange::decode_structure(const structured::Keys& keys,
    ...
    switch (sty)
    {
        ...
        case timerange::Style::GRIB2:
            return createGRIB2(
                ...
                val.as_int(keys.timerange_p2, "timerange p2"));
        case timerange::Style::TIMEDEF: {
            uint32_t step_len = val.as_int(keys.timerange_step_len,
                                           "Timedef forecast step length");
            auto step_unit    = static_cast<timerange::TimedefUnit>(val.as_int(
                keys.timerange_step_unit, "Timedef forecast step units"));

            int stat_type     = 255;
            uint32_t stat_len = 0;
            auto stat_unit    = timerange::UNIT_MISSING;

            if (val.has_key(keys.timerange_stat_type,
                            structured::NodeType::INT))
            {
                stat_type = val.as_int(keys.timerange_stat_type,
                                       "Timedef statistical type");

                if (val.has_key(keys.timerange_stat_unit,
                                structured::NodeType::INT))
                {
                    stat_unit = static_cast<timerange::TimedefUnit>(val.as_int(
                        keys.timerange_stat_unit, "Timedef statistical unit"));
                    stat_len  = val.as_int(
                        keys.timerange_stat_len,
                        "Timedef length of interval of statistical processing");
                }
            }
...
std::unique_ptr<Timerange> Timerange::createGRIB1(unsigned char type,
                                                  unsigned char unit,
                                                  unsigned char p1,
                                                  unsigned char p2)
{
...
std::unique_ptr<Timerange> Timerange::createGRIB2(unsigned char type,
                                                  unsigned char unit,
                                                  signed long p1,
                                                  signed long p2)
{
...
std::unique_ptr<Timerange>
Timerange::createTimedef(uint32_t step_len, timerange::TimedefUnit step_unit)
{
    // Normalize input
    step_unit = step_unit == timerange::UNIT_MISSING ? timerange::UNIT_MISSING
                : step_len == 0                      ? timerange::UNIT_SECOND
                                                     : step_unit;
                         ...
                         timerange::TimedefUnit stat_unit)
{
    ...
    step_unit = step_unit == timerange::UNIT_MISSING ? timerange::UNIT_MISSING
                : step_len == 0                      ? timerange::UNIT_SECOND
                                                     : step_unit;
    stat_unit = stat_unit == timerange::UNIT_MISSING ? timerange::UNIT_MISSING
                : stat_len == 0                      ? timerange::UNIT_SECOND
                                                     : stat_unit;
...
std::unique_ptr<Timerange> Timerange::createBUFR(unsigned value,
                                                 unsigned char unit)
{
    // TODO: optimize encoding by precomputing buffer size and not using a
    // vector? to do that, we first need a function that estimates the size of
...
int GRIB1::compare_local(const GRIB1& o) const
{
    ...
    if (int res = atype - btype)
        return res;
    if (int res = aunit - bunit)
        return res;
    if (int res = ap1 - bp1)
        return res;
    return ap2 - bp2;
...
std::ostream& GRIB1::writeNumbers(std::ostream& o) const
{
    ...
    switch ((t_enum_GRIB_TIMERANGE)type)
    {
        ...
        case GRIB_TIMERANGE_ANALYSIS_AT_REFTIME:                                  o << setw(3) << type; break;
        case GRIB_TIMERANGE_VALID_IN_REFTIME_PLUS_P1_REFTIME_PLUS_P2:
        case GRIB_TIMERANGE_AVERAGE_IN_REFTIME_PLUS_P1_REFTIME_PLUS_P2:
        case GRIB_TIMERANGE_ACCUMULATED_INTERVAL_REFTIME_PLUS_P1_REFTIME_PLUS_P2:
        case GRIB_TIMERANGE_DIFFERENCE_REFTIME_PLUS_P2_REFTIME_PLUS_P1:
...
bool GRIB1::get_forecast_step(int& step, bool& is_seconds) const
{
    ...
    int timemul;
    is_seconds = GRIB1_get_timeunit_conversion(
        static_cast<t_enum_GRIB_TIMEUNIT>(unit), timemul);
    ...
    switch (type)
    {
        ...
        case GRIB_TIMERANGE_VALID_IN_REFTIME_PLUS_P1_REFTIME_PLUS_P2: // = 2
            // ELSE IF (tri == 2) THEN ! somewhere between p1 and p2, not good
            // for grib2 standard
            //   statproc = 205
        ...
        case GRIB_TIMERANGE_COSMO_NUDGING: // = 13
            // ELSE IF (tri == 13) THEN ! COSMO-nudging, use a temporary value
            // then normalize
            ...
            // ! recognized as a COSMO statistically processed parameter (and in
            // case ! of maximum or minimum the parameter is corrected as well);
            // if ! parameter is not recognized, it is converted to
            // instantaneous with a ! warning.
            //            return 206;
        ...
        case GRIB_TIMERANGE_AVERAGE_OF_SUCCESSIVE_FORECAST_AVERAGES: //                  = 131
            // call l4f_log(L4F_ERROR,'timerange_g1_to_g2: GRIB1 timerange
            // '//TRIM(to_char(tri)) &
            ...
            //   call l4f_log(L4F_WARN,"inconsistence in
            //   timerange:254,"//trim(to_char(p1))//","//trim(to_char(p2)))
            // end if
    }
    return false;
}
...
void GRIB1::arg_significance(unsigned type, bool& use_p1, bool& use_p2)
{
    ...
    switch ((t_enum_GRIB_TIMERANGE)type)
    {
        case GRIB_TIMERANGE_FORECAST_AT_REFTIME_PLUS_P1: use_p1 = true; break;
        case GRIB_TIMERANGE_ANALYSIS_AT_REFTIME:         break;
        case GRIB_TIMERANGE_VALID_IN_REFTIME_PLUS_P1_REFTIME_PLUS_P2:
        case GRIB_TIMERANGE_AVERAGE_IN_REFTIME_PLUS_P1_REFTIME_PLUS_P2:
        case GRIB_TIMERANGE_ACCUMULATED_INTERVAL_REFTIME_PLUS_P1_REFTIME_PLUS_P2:
...
std::ostream& GRIB2::writeToOstream(std::ostream& o) const
{
    ...
    return o << formatStyle(Style::GRIB2) << "(" << setfill('0') << internal
             << setw(3) << ty << ", " << setw(3) << un << ", " << setw(10) << p1
             << suffix << ", " << setw(10) << p2 << suffix << ")";
}
...
std::string GRIB2::exactQuery() const
{
    ...
    std::stringstream o;
    o << formatStyle(Style::GRIB2) << "," << ty << "," << un << "," << p1 << ","
      << p2;
    return o.str();
}
...
int GRIB2::get_proc_type() const { return -1; }
...
bool Timedef::equals(const Type& o) const
{
    ...
    if (step_unit == UNIT_MISSING && vstep_unit != UNIT_MISSING)
        return false;
    if (step_unit != UNIT_MISSING && vstep_unit == UNIT_MISSING)
        return false;
    if (step_unit != UNIT_MISSING)
    {
        ...
        if (is1 != is2)
            return false;
        if (step_len * timemul1 != vstep_len * timemul2)
            return false;
    }

    // Compare type of statistical processing
    if (stat_type != vstat_type)
        return false;

    // Normalise stat units and compare stat processing length
    if (stat_unit == UNIT_MISSING && vstat_unit != UNIT_MISSING)
        return false;
    if (stat_unit != UNIT_MISSING && vstat_unit == UNIT_MISSING)
        return false;
    if (stat_unit != UNIT_MISSING)
    {
        ...
        if (is1 != is2)
            return false;
        if (stat_len * timemul1 != vstat_len * timemul2)
            return false;
    }
...
int Timedef::compare_local(const Timedef& o) const
{
    ...
    // Normalise step time units and compare steps
    if (step_unit == 255 && vstep_unit != 255)
        return -1;
    if (step_unit != 255 && vstep_unit == 255)
        return 1;
    if (step_unit != 255)
    {
        ...
        if (is2 && !is1)
            return 1;
        if (int res = step_len * timemul1 - vstep_len * timemul2)
            return res;
    }

    // Compare type of statistical processing
    if (int res = stat_type - vstat_type)
        return res;

    // Normalise stat units and compare stat processing length
    if (stat_unit == 255 && vstat_unit != 255)
        return -1;
    if (stat_unit != 255 && vstat_unit == 255)
        return 1;
    if (stat_unit != 255)
    {
        ...
        if (is2 && !is1)
            return 1;
        if (int res = stat_len * timemul1 - vstat_len * timemul2)
            return res;
    }
...
std::ostream& Timedef::writeToOstream(std::ostream& o) const
{
    ...
    o << formatStyle(Style::TIMEDEF) << "(" << setfill('0') << internal;
...
void Timedef::serialise_local(structured::Emitter& e,
                              const structured::Keys& keys,
                              const Formatter* f) const
{
    timerange::TimedefUnit step_unit, stat_unit;
...
bool Timedef::get_proc_duration(int& duration, bool& is_seconds) const
{
    ...
    if (stat_type == 255 || stat_unit == UNIT_MISSING)
        return false;
...
Timedef::validity_time_to_emission_time(const reftime::Position& src) const
{
    ...
    if (!can_compute)
        throw_consistency_error("converting validity time to emission time",
                                "cannot compute the forecast step");
    if (!is_secs)
        throw_consistency_error(
            "converting validity time to emission time",
            "timedef has a step that cannot be converted to seconds");
...
bool Timedef::timeunit_conversion(TimedefUnit unit, int& timemul)
{
    ...
    switch (unit)
    {
        ...
        case UNIT_MISSING:
            throw_consistency_error("normalising time",
                                    "time unit is missing (255)");
        default: {
            std::stringstream ss;
...
const char* Timedef::timeunit_suffix(TimedefUnit unit)
{
    switch (unit)
    {
        ...
        case UNIT_MISSING:
            throw_consistency_error("finding time unit suffix",
                                    "time unit is missing (255)");
        default: {
            std::stringstream ss;
            ss << "cannot find find time unit suffix: time unit is unknown ("
               << (int)unit << ")";
            throw std::runtime_error(ss.str());
        }
    }
...
bool BUFR::equals(const Type& o) const
{
    ...
    if (is_seconds(un) != v->is_seconds(vun))
        return false;
    if (is_seconds(un))
...
int BUFR::compare_local(const BUFR& o) const
{
    ...
    if (int res = (is_seconds(un) ? 0 : 1) - (o.is_seconds(vun) ? 0 : 1))
        return res;
    if (is_seconds(un))
...
int BUFR::get_proc_type() const { return -1; }
...
bool BUFR::is_seconds(unsigned unit)
{
    switch ((t_enum_GRIB_TIMEUNIT)unit)
    {
        ...
        case GRIB_TIMEUNIT_SECOND:  return true;
        case GRIB_TIMEUNIT_MONTH:
        case GRIB_TIMEUNIT_YEAR:
        case GRIB_TIMEUNIT_DECADE:
        case GRIB_TIMEUNIT_NORMAL:
        case GRIB_TIMEUNIT_CENTURY: return false;
        default:                    {
            std::stringstream ss;
...
unsigned BUFR::seconds(unsigned unit, unsigned value)
{
    ...
    switch ((t_enum_GRIB_TIMEUNIT)unit)
    {
        ...
        default:                    {
            std::stringstream ss;
            ss << "cannot normalise TimeRange: time unit (" << unit
               << ") does not convert to seconds";
            throw std::runtime_error(ss.str());
        }
    }
}
...
void Timerange::init() { MetadataType::register_type<Timerange>(); }
