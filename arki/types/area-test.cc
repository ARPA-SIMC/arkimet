...
void Tests::register_tests()
{
    ...
    add_generic_test("grib", {},
                     "GRIB(uno=1,due=2,tre=-3,supercazzola=-1234567,pippo="
                     "pippo,pluto=\"12\",cippo=)",
                     {
                         "GRIB(dieci=10,undici=11,dodici=-12)",
                     },
                     "GRIB:cippo=, due=2, pippo=pippo, pluto=\"12\", "
                     "supercazzola=-1234567, tre=-3, uno=1");

    // Check two more samples
    add_generic_test("grib_1", {}, "GRIB(count=1,pippo=pippo)",
                     {
    ...
    add_generic_test("odimh5", {},
                     "ODIMH5(uno=1,due=2,tre=-3,supercazzola=-1234567,pippo="
                     "pippo,pluto=\"12\",cippo=)",
                     {
                         "ODIMH5(dieci=10,undici=11,dodici=-12)",
                     },
                     "ODIMH5:cippo=, due=2, pippo=pippo, pluto=\"12\", "
                     "supercazzola=-1234567, tre=-3, uno=1");

    add_generic_test("vm2", {}, "VM2(1)",
                     {
                         "VM2(2)",
                     },
                     "VM2,1");
    ...
    add_method("vm2_derived_lookup", [] {
        ...
        std::unique_ptr<Type> p2;
        {
            ...
            p2 = Type::decodeInner(arki::TYPE_AREA, dec);
        }
    ...
    add_method("vm2_derived_encoding", [] {
        ...
        // Encode with derived values: encodeWithoutEnvelope adds the derived
        // values
        std::vector<uint8_t> full;
        {
            core::BinaryEncoder enc(full);
        ...
        {
            core::BinaryDecoder dec(full);
            pfull = Type::decodeInner(arki::TYPE_AREA, dec);
        }
        {
            core::BinaryDecoder dec(part);
            ppart = Type::decodeInner(arki::TYPE_AREA, dec);
        }
