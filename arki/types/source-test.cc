...
void Tests::register_tests()
{
    ...
    add_generic_test("blob",
                     {
                         "BLOB(grib,testfile:100+50)",
                         "BLOB(bufr,festfile:100+50)",
                         "BLOB(bufr,testfile:99+50)",
                         "BLOB(bufr,testfile:100+49)",
                     },
                     "BLOB(bufr,testfile:100+50)",
                     {
                         "BLOB(vm2,testfile:100+50)",
                         "BLOB(bufr,zestfile:100+50)",
                         "BLOB(bufr,testfile:101+50)",
                         "BLOB(bufr,testfile:100+51)",
                         "INLINE(bufr,9)",
                         "URL(bufr,foo)",
                     });
    ...
    add_method("blob_details", [] {
        unique_ptr<Source> o = Source::createBlobUnlocked(DataFormat::JPEG, "",
                                                          "testfile", 21, 42);
        wassert(actual(o).is_source_blob(DataFormat::JPEG, "", "testfile", 21u,
                                         42u));
	...
	ensure_equals(size, 42u);
        ...
        wassert(actual(o) == Source::createBlobUnlocked(DataFormat::JPEG, "",
                                                        "testfile", 21, 42));
        wassert(actual(o) == Source::createBlobUnlocked(
                                 DataFormat::JPEG, "/tmp", "testfile", 21, 42));
        wassert(actual(o) != Source::createBlobUnlocked(DataFormat::JPEG, "",
                                                        "testfile", 22, 43));
    ...
    add_method("blob_bigfiles", [] {
        unique_ptr<Source> o = Source::createBlobUnlocked(
            DataFormat::ODIMH5, "", "testfile", 0x8000FFFFffffFFFFLLU, 42);
        wassert(actual(o).is_source_blob(DataFormat::ODIMH5, "", "testfile",
                                         0x8000FFFFffffFFFFLLU, 42u));
    ...
    add_method("blob_pathnames", [] {
        unique_ptr<source::Blob> o = source::Blob::create_unlocked(
            DataFormat::VM2, "", "testfile", 21, 42);
        wassert(actual(o->absolutePathname()) == "testfile");

        o = source::Blob::create_unlocked(DataFormat::VM2, "/tmp", "testfile",
                                          21, 42);
        wassert(actual(o->absolutePathname()) == "/tmp/testfile");

        o = source::Blob::create_unlocked(DataFormat::VM2, "/tmp",
                                          "/antani/testfile", 21, 42);
    ...
    add_method("blob_pathnames_encode", [] {
        unique_ptr<Source> o = Source::createBlobUnlocked(
            DataFormat::NETCDF, "/tmp", "testfile", 21, 42);
        ...
        wassert(actual(decoded).is_source_blob(DataFormat::NETCDF, "",
                                               "testfile", 21, 42));
        ...
        // Encode to JSON, decode, we keep the root
        {
            ...
            decoded = downcast<Source>(
                types::decode_structure(structured::keys_json, parsed.root()));
            wassert(actual(decoded).is_source_blob(DataFormat::NETCDF, "/tmp",
                                                   "testfile", 21, 42));
        }
    });
    ...
    add_method("blob_stream", [] {
        ...
        std::filesystem::remove("test.grib");
        {
            auto stream = StreamOutput::create(std::make_shared<File>(
                "test.grib", O_WRONLY | O_CREAT | O_TRUNC));
    ...
    add_method("url_details", [] {
        unique_ptr<Source> o =
            Source::createURL(DataFormat::GRIB, "http://foobar");
        wassert(actual(o).is_source_url(DataFormat::GRIB, "http://foobar"));

        wassert(actual(o) ==
                Source::createURL(DataFormat::GRIB, "http://foobar"));
        wassert(actual(o) !=
                Source::createURL(DataFormat::GRIB, "http://foobar/a"));
        wassert(actual(o) != Source::createBlobUnlocked(DataFormat::GRIB, "",
                                                        "http://foobar", 1, 2));
    ...
    add_method("inline_details", [] {
        ...
        wassert(actual(o) !=
                Source::createBlobUnlocked(DataFormat::GRIB, "", "", 0, 12344));
