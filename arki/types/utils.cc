...
MetadataType::MetadataType(types::Code type_code, int serialisationSizeLen,
    ...
    : type_code(type_code), serialisationSizeLen(serialisationSizeLen),
      tag(tag), decode_func(decode_func),
      string_decode_func(string_decode_func),
      structure_decode_func(structure_decode_func), document_func(document_func)
{
}
...
const MetadataType* MetadataType::get(types::Code code)
{
    ...
    {
        stringstream ss;
        ss << "cannot parse binary data: no decoder found for item type "
           << code;
...
void MetadataType::document_types(stream::Text& out, unsigned heading_level)
{
    if (!decoders)
        throw std::runtime_error(
            "Metadata item types have not been registered");
