...
{
    if (str == "POSITION")
        return reftime::Style::POSITION;
    if (str == "PERIOD")
        return reftime::Style::PERIOD;
    throw_consistency_error("parsing Reftime style",
                            "cannot parse Reftime style '" + str +
                                "': only POSITION and PERIOD are supported");
}
...
void Reftime::get_Period(const uint8_t* data, unsigned size, core::Time& begin,
                         core::Time& end)
{
    core::BinaryDecoder dec(data + 1, size - 1);
...
Reftime::decode_structure(const structured::Keys& keys,
    ...
    switch (sty)
    {
        case Style::POSITION:
            return createPosition(
                val.as_time(keys.reftime_position_time, "time"));
...
void Position::serialise_local(structured::Emitter& e,
...
void Reftime::init() { MetadataType::register_type<Reftime>(); }
