...
unsigned Level::GRIB1_type_vals(unsigned char type)
{
    switch ((t_enum_GRIB_LEVELS)type)
    {
        ...
        case GRIB_LEVEL_MEAN_SEA_LEVEL:                             return 0;
...
Level::Style Level::parseStyle(const std::string& str)
{
    ...
    throw_consistency_error(
        "parsing Level style",
        "cannot parse Level style '" + str +
            "': only GRIB1, GRIB2S, GRIB2D, ODIMH5 are supported");
}
...
Level* Level::clone() const { return new Level(data, size); }
...
int Level::compare(const Type& o) const
{
    ...
    switch (sty)
    {
        ...
        case level::Style::GRIB2S: {
            ...
            if (int res = compare_with_missing(ty, vty, GRIB2_MISSING_TYPE))
                return res;
            if (int res = compare_with_missing(sc, vsc, GRIB2_MISSING_SCALE))
                return res;
        ...
        case level::Style::GRIB2D: {
            ...
            // FIXME: here we can handle uniforming the scales if needed
            if (int res = compare_with_missing(ty1, vty1, GRIB2_MISSING_TYPE))
                return res;
            if (int res = compare_with_missing(sc1, vsc1, GRIB2_MISSING_SCALE))
                return res;
            if (int res = compare_with_missing(va1, vva1, GRIB2_MISSING_VALUE))
                return res;
            if (int res = compare_with_missing(ty2, vty2, GRIB2_MISSING_TYPE))
                return res;
            if (int res = compare_with_missing(sc2, vsc2, GRIB2_MISSING_SCALE))
                return res;
        ...
        case level::Style::ODIMH5: {
            ...
            if (int res = compare_double(vmin, vvmin))
                return res;
        ...
        default:
            throw_consistency_error("parsing Level",
                                    "unknown Level style " + formatStyle(sty));
    }
}
...
void Level::get_GRIB1(const uint8_t* data, unsigned size, unsigned& type,
...
void Level::get_GRIB2S(const uint8_t* data, unsigned size, unsigned& type,
...
void Level::get_GRIB2D(const uint8_t* data, unsigned size, unsigned& type1,
...
void Level::get_ODIMH5(const uint8_t* data, unsigned size, double& min,
                       double& max)
{
    core::BinaryDecoder dec(data + 1, size - 1);
...
std::ostream& Level::writeToOstream(std::ostream& o) const
{
    ...
    switch (sty)
    {
        ...
        case level::Style::ODIMH5: {
            ...
            utils::SaveIOState sios(o);
            return o << formatStyle(style()) << "(" << std::setprecision(5)
                     << vmin << ", " << std::setprecision(5) << vmax << ")";
        }
        default:
            throw_consistency_error("parsing Level",
                                    "unknown Level style " + formatStyle(sty));
...
void Level::serialise_local(structured::Emitter& e,
                            ...
                            const Formatter* f) const
{
    ...
    switch (sty)
    {
        case level::Style::GRIB1: {
            ...
            {
                case 0: break;
                case 1: e.add(keys.level_l1, l1); break;
                case 2:
    ...
    switch (sty)
    {
        case level::Style::GRIB1: {
            ...
            switch (GRIB1_type_vals(ty))
            {
                ...
                default:
                    snprintf(buf, 128, "GRIB1,%u,%u,%u", ty, l1, l2);
                    break;
            }
            return buf;
        }
        ...
        case level::Style::ODIMH5: {
            ...
            ss << "ODIMH5,range " << std::setprecision(5) << vmin << " "
...
static int getNumber(const char*& start, const char* what)
{
    ...
    if (!*start)
        throw_consistency_error("parsing Level",
                                string("no ") + what + " after level type");

    int res = strtol(start, &endptr, 10);
    if (endptr == start)
        throw_consistency_error("parsing Level", string("expected ") + what +
                                                     ", but found \"" + start +
                                                     "\"");
    start = endptr;
...
unique_ptr<Level> Level::decodeString(const std::string& val)
{
    ...
    switch (style)
    {
        ...
        case Style::GRIB2S: {
            ...
            uint8_t type =
                getUnsigned<uint8_t>(start, "level type", GRIB2_MISSING_TYPE);
            uint8_t scale  = getUnsigned<uint8_t>(start, "scale of level value",
                                                  GRIB2_MISSING_SCALE);
            uint32_t value = getUnsigned<uint32_t>(start, "level value",
                                                   GRIB2_MISSING_VALUE);
        ...
        case Style::GRIB2D: {
            ...
            uint8_t type1   = getUnsigned<uint8_t>(start, "type of first level",
                                                   GRIB2_MISSING_TYPE);
            uint32_t scale1 = getUnsigned<uint32_t>(
                start, "scale of value of first level", GRIB2_MISSING_SCALE);
            uint32_t value1 = getUnsigned<uint32_t>(
                start, "value of first level", GRIB2_MISSING_VALUE);
            uint8_t type2 = getUnsigned<uint8_t>(start, "type of second level",
                                                 GRIB2_MISSING_TYPE);
            uint32_t scale2 = getUnsigned<uint32_t>(
                start, "scale of value of second level", GRIB2_MISSING_SCALE);
            uint32_t value2 = getUnsigned<uint32_t>(
                start, "value of second level", GRIB2_MISSING_VALUE);
        ...
        default:
            throw_consistency_error("parsing Level", "unknown Level style " +
                                                         formatStyle(style));
    }
}

std::unique_ptr<Level> Level::decode_structure(const structured::Keys& keys,
                                               const structured::Reader& val)
{
    ...
    switch (sty)
    {
            ...
            switch (GRIB1_type_vals(lt))
            {
                ...
                default:
                    throw std::invalid_argument(
                        "unsupported level type value " + std::to_string(lt));
            }
        }
        case level::Style::GRIB2S: {
            int lt = GRIB2_MISSING_TYPE, sc = GRIB2_MISSING_SCALE,
                va = GRIB2_MISSING_VALUE;
        ...
        case level::Style::GRIB2D: {
            int l1 = GRIB2_MISSING_TYPE, s1 = GRIB2_MISSING_SCALE,
                v1 = GRIB2_MISSING_VALUE;
            ...
            int l2 = GRIB2_MISSING_TYPE, s2 = GRIB2_MISSING_SCALE,
                v2 = GRIB2_MISSING_VALUE;
        ...
        case level::Style::ODIMH5:
            return createODIMH5(val.as_double(keys.level_min, "level min"),
...
std::unique_ptr<Level> Level::createGRIB2S(uint8_t type, uint8_t scale,
                                           uint32_t val)
{
#ifdef HAVE_GRIBAPI
...
std::unique_ptr<Level> Level::createGRIB2D(uint8_t type1, uint8_t scale1,
                                           ...
                                           uint8_t scale2, uint32_t val2)
{
    ...
    enc.add_unsigned(type1, 1);
    enc.add_unsigned(scale1, 1);
    enc.add_varint(val1);
    enc.add_unsigned(type2, 1);
    enc.add_unsigned(scale2, 1);
    enc.add_varint(val2);
    return std::unique_ptr<Level>(new Level(buf));
}
...
void Level::init() { MetadataType::register_type<Level>(); }
