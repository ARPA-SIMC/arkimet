...
BBox::Style BBox::parseStyle(const std::string& str)
{
    ...
    if (str == "HULL")
        return Style::HULL;
    throw_consistency_error("parsing BBox style",
                            "cannot parse BBox style '" + str +
                                "': only INVALID and BOX are supported");
}
...
bbox::Style BBox::style() const { return (bbox::Style)data[0]; }
...
std::unique_ptr<BBox> BBox::decode_structure(const structured::Keys& keys,
...
void BBox::serialise_local(structured::Emitter& e, const structured::Keys& keys,
                           const Formatter* f) const
{
    e.add(keys.type_style, formatStyle(Style::INVALID));
}

void BBox::init() { MetadataType::register_type<BBox>(); }
