...
int Origin::compare(const Type& o) const
{
    ...
    switch (sty)
    {
        ...
        case origin::Style::ODIMH5: {
            ...
            if (int res = WMO.compare(vWMO))
                return res;
            if (int res = RAD.compare(vRAD))
                return res;
            return PLC.compare(vPLC);
        }
        default:
            throw_consistency_error("parsing Origin",
                                    "unknown Origin style " + formatStyle(sty));
    }
}
...
void Origin::get_GRIB1(const uint8_t* data, unsigned size, unsigned& centre,
...
void Origin::get_GRIB2(const uint8_t* data, unsigned size, unsigned& centre,
...
void Origin::get_BUFR(const uint8_t* data, unsigned size, unsigned& centre,
                      unsigned& subcentre)
{
    core::BinaryDecoder dec(data + 1, size - 1);
...
void Origin::get_ODIMH5(const uint8_t* data, unsigned size, std::string& WMO,
...
{
    if (str == "GRIB1")
        return origin::Style::GRIB1;
    if (str == "GRIB2")
        return origin::Style::GRIB2;
    if (str == "BUFR")
        return origin::Style::BUFR;
    if (str == "ODIMH5")
        return origin::Style::ODIMH5;
    throw_consistency_error("parsing Origin style",
                            "cannot parse Origin style '" + str +
...
std::ostream& Origin::writeToOstream(std::ostream& o) const
{
    ...
    switch (sty)
    {
        case origin::Style::GRIB1: {
            unsigned c, s, p;
            get_GRIB1(c, s, p);
            return o << formatStyle(sty) << "(" << setfill('0') << setw(3)
                     << (int)c << ", " << setw(3) << (int)s << ", " << setw(3)
                     << (int)p << setfill(' ') << ")";
        }
        case origin::Style::GRIB2: {
            unsigned ce, su, pt, bg, pi;
            get_GRIB2(ce, su, pt, bg, pi);
            return o << formatStyle(sty) << "(" << setfill('0') << setw(5)
                     << (int)ce << ", " << setw(5) << (int)su << ", " << setw(3)
                     << (int)pt << ", " << setw(3) << (int)bg << ", " << setw(3)
                     << (int)pi << setfill(' ') << ")";
        }
        case origin::Style::BUFR: {
            unsigned c, s;
            get_BUFR(c, s);
            return o << formatStyle(sty) << "(" << setfill('0') << setw(3)
                     << (int)c << ", " << setw(3) << (int)s << setfill(' ')
                     << ")";
        }
        case origin::Style::ODIMH5: {
            std::string WMO, RAD, PLC;
            get_ODIMH5(WMO, RAD, PLC);
            return o << formatStyle(sty) << "(" << WMO << ", " << RAD << ", "
                     << PLC << ")";
        }
        default:
            throw_consistency_error("parsing Origin",
...
void Origin::serialise_local(structured::Emitter& e,
...
std::unique_ptr<Origin> Origin::decodeString(const std::string& val)
{
    ...
    switch (sty)
    {
        ...
        case origin::Style::ODIMH5: {
            ...
            for (str::Split::const_iterator i = split.begin(); i != split.end();
                 ++i)
                values.push_back(*i);
...
std::unique_ptr<Origin> Origin::decode_structure(const structured::Keys& keys,
    ...
    switch (sty)
    {
        ...
        case origin::Style::GRIB2:
            return createGRIB2(
                ...
                val.as_int(keys.origin_background_process_id,
                           "origin bg process id"),
        ...
        case origin::Style::BUFR:
            return createBUFR(
                ...
                val.as_int(keys.origin_subcentre, "origin subcentre"));
        case origin::Style::ODIMH5:
            return createODIMH5(val.as_string(keys.origin_wmo, "origin wmo"),
...
std::string Origin::exactQuery() const
{
    switch (style())
    {
        ...
        case origin::Style::GRIB2: {
            ...
            get_GRIB2(ce, su, pt, bg, pi);
            char buf[64];
            snprintf(buf, 64, "GRIB2,%d,%d,%d,%d,%d", (int)ce, (int)su, (int)pt,
                     (int)bg, (int)pi);
            return buf;
        }
        case origin::Style::BUFR: {
        ...
        default:
            throw_consistency_error("parsing Origin", "unknown Origin style " +
                                                          formatStyle(style()));
    }
}

std::unique_ptr<Origin> Origin::createGRIB1(unsigned char centre,
...
std::unique_ptr<Origin> Origin::createGRIB2(unsigned short centre,
...
std::unique_ptr<Origin> Origin::createBUFR(unsigned char centre,
                                           unsigned char subcentre)
{
...
std::unique_ptr<Origin> Origin::createODIMH5(const std::string& wmo,
...
void Origin::init() { MetadataType::register_type<Origin>(); }
