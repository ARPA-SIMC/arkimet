...
public:
    ...
    {
        return traits<Quantity>::type_code;
    }
    size_t serialisationSizeLength() const override
    {
        return traits<Quantity>::type_sersize_bytes;
    }
    std::string tag() const override { return traits<Quantity>::type_tag; }

    // TODO: use a sorted vector instead of a std::set, since we always store
    // few values
    ...
    static std::unique_ptr<Quantity>
    create(const std::set<std::string>& values);
    static std::unique_ptr<Quantity>
