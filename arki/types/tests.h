...
struct TestGenericType
{
    ...
    /// Decode item from encoded, running single-item checks and passing
    /// ownership back the caller
...
{
public:
    ActualType(const types::Type* actual)
        : arki::utils::tests::Actual<const types::Type*>(actual)
    {
    }
    ...
    template <typename E>
    void operator==(const std::unique_ptr<E>& expected) const
    {
        operator==(expected.get());
    }
    template <typename E>
    void operator!=(const std::unique_ptr<E>& expected) const
    {
    ...
    void operator!=(const std::string& expected) const;
    /*
    template<typename E> TestIsLt<A, E> operator<(const E& expected) const {
    return TestIsLt<A, E>(actual, expected); } template<typename E> TestIsLte<A,
    E> operator<=(const E& expected) const { return TestIsLte<A, E>(actual,
    expected); } template<typename E> TestIsGt<A, E> operator>(const E&
    expected) const { return TestIsGt<A, E>(actual, expected); }
    template<typename E> TestIsGte<A, E> operator>=(const E& expected) const {
    return TestIsGte<A, E>(actual, expected); }
    */
    ...
    void is_source_blob(DataFormat format, const std::filesystem::path& basedir,
                        const std::filesystem::path& fname, uint64_t ofs,
                        uint64_t size);
...
inline arki::tests::ActualType actual_type(const arki::types::Type& actual)
{
    return arki::tests::ActualType(&actual);
}
inline arki::tests::ActualType actual_type(const arki::types::Type* actual)
{
    return arki::tests::ActualType(actual);
}
inline arki::tests::ActualType actual(const arki::types::Type& actual)
{
    return arki::tests::ActualType(&actual);
}
template <typename T>
inline arki::tests::ActualType actual(const std::unique_ptr<T>& actual)
{
    return arki::tests::ActualType(actual.get());
}
inline arki::tests::Actual<const types::ValueBag&>
actual(const types::ValueBag& actual)
{
    return arki::tests::Actual<const types::ValueBag&>(actual);
}

template <typename TYPE> struct TypeTestCase : public TestCase
{
    using TestCase::TestCase;

    static std::unique_ptr<TYPE> parse(const std::string& encoded)
    {
        auto res = wcallchecked(
            types::decodeString(types::traits<TYPE>::type_code, encoded));
        return downcast<TYPE>(move(res));
    }

    void add_generic_test(const char* name,
                          const std::vector<std::string>& lower,
                          const std::vector<std::string>& exact,
                          const std::vector<std::string>& higher,
                          const std::string& exact_match = std::string())
    {
        add_method(name, [=] {
    ...
    void add_generic_test(const char* name,
                          ...
                          const std::string& exact_match = std::string())
    {
        add_generic_test(name, lower, std::vector<std::string>({exact}), higher,
                         exact_match);
    }
};
