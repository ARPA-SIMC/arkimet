...
void Tests::register_tests()
{
    ...
    add_method("typeset", []() {
        ...
        // Lookup
        wassert(actual(v.find(*decodeString(TYPE_REFTIME,
                                            "2015-01-05T12:00:00Z")) == sample)
                    .istrue());
        wassert(actual(v.find(*decodeString(TYPE_REFTIME,
                                            "2015-01-05T11:00:00Z")) == sample)
                    .isfalse());
        ...
        // Iteration
        {
            ...
            wassert(
                actual_type(*i).is_reftime_position({2015, 1, 5, 12, 0, 0}));
            ++i;
            wassert(actual(i == v.end()).istrue());
        }

        // Insert again, we get the previous pointer
        wassert(actual(v.insert(decodeString(TYPE_REFTIME,
                                             "2015-01-05T12:00:00Z")) == sample)
                    .istrue());
