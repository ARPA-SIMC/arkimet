...
void Source::serialise_local(structured::Emitter& e, const structured::Keys& keys, const Formatter* f) const
{
    ...
    e.add(keys.source_format); e.add(format_name(format));
}
...
unique_ptr<Source> Source::decodeRelative(core::BinaryDecoder& dec, const std::filesystem::path& basedir)
{
    ...
    DataFormat format = format_from_string(dec.pop_string(format_len, "source format name"));
    switch (s)
    {
...
unique_ptr<Source> Source::createBlob(DataFormat format, const std::filesystem::path& basedir, const std::filesystem::path& filename, uint64_t offset, uint64_t size, std::shared_ptr<segment::data::Reader> reader)
{
    return upcast<Source>(source::Blob::create(format, basedir, filename, offset, size, reader));
}

unique_ptr<Source> Source::createBlobUnlocked(DataFormat format, const std::filesystem::path& basedir, const std::filesystem::path& filename, uint64_t offset, uint64_t size)
{
