...
Area::Style Area::parseStyle(const std::string& str)
{
    ...
    if (str == "VM2")
        return Style::VM2;
    throw_consistency_error("parsing Area style",
                            "cannot parse Area style '" + str +
                                "': only GRIB,ODIMH5 is supported");
}
...
int Area::compare(const Type& o) const
{
    ...
    switch (sty)
    {
        ...
        default:
            throw_consistency_error("parsing Area",
                                    "unknown Area style " + formatStyle(sty));
    }
}
    ...
    switch (sty)
    {
        ...
        default:
            throw std::runtime_error(
                "cannot parse Area: style is " + formatStyle(sty) +
                " but we can only decode GRIB, ODIMH5 and VM2");
    }
    return res;
}
...
std::unique_ptr<Area> Area::decode_structure(const structured::Keys& keys,
                                             const structured::Reader& val)
{
    ...
    {
        case Style::GRIB:
            val.sub(keys.area_value, "area value",
...
void GRIB::serialise_local(structured::Emitter& e, const structured::Keys& keys,
...
std::string GRIB::exactQuery() const { return "GRIB:" + get_GRIB().toString(); }
...
int VM2::compare_local(const VM2& o) const { return get_VM2() - o.get_VM2(); }
...
void Area::init() { MetadataType::register_type<Area>(); }
