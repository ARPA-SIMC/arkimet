...
void Inline::serialise_local(structured::Emitter& e,
...
std::unique_ptr<Inline>
Inline::decode_structure(const structured::Keys& keys,
                         const structured::Reader& reader)
{
    return Inline::create(format_from_string(reader.as_string(
                              keys.source_format, "source format")),
...
int Inline::compare_local(const Source& o) const
{
    ...
    if (!v)
        throw_consistency_error(
            ...
            string("second element claims to be a Inline Source, but is a ") +
...
Inline* Inline::clone() const { return new Inline(*this); }
