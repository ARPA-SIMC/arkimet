...
MockEngine::MockEngine()
{
    ...
    else
    {
        throw std::runtime_error(
            "ARKI_MOCK_SCAN_DB not defined but needed by arkimet mock scanner");
    }
...
std::string compute_hash(const char* name, const void* data, size_t size)
{
    ...
    if (!md)
        throw std::invalid_argument(
            std::string("checksum algorithm not found: ") + name);
...
std::shared_ptr<Metadata> MockEngine::by_checksum(const std::string& checksum)
{
    ...
    {
        const uint8_t* buf =
            static_cast<const uint8_t*>(by_sha256sum->fetchBlob(0));
        int len = by_sha256sum->fetchBytes(0);
        core::MemoryFile in(buf, len, db_pathname);
        arki::metadata::AbstractFileBinaryReader md_reader(in);
        md    = md_reader.read(false);
        found = true;
    }
    if (!found)
        throw std::invalid_argument("data " + checksum +
                                    " not found in mock scan database");
    return md;
}
