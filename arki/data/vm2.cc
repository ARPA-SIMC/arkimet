                       ...
                       size_t size) const override
    {
        if (size >= 1024)
            throw_check_error(fd, offset,
                              "size of data to check (" + std::to_string(size) +
                                  ") is too long for a VM2 line");
...
public:
    ...
    Input(std::istream& st) : in(&st), close(false)
    {
        parser = new meteo::vm2::Parser(*in);
    }
...
Scanner::scan_singleton(const std::filesystem::path& abspath)
{
    ...
    if (input.next())
        throw std::runtime_error(abspath.native() +
                                 " contains more than one VM2 data");
    return md;
}
...
bool Scanner::scan_pipe(core::NamedFileDescriptor& in, metadata_dest_func dest)
{
    using namespace std;
    // see
    // https://stackoverflow.com/questions/2746168/how-to-construct-a-c-fstream-from-a-posix-file-descriptor#5253726
    ...
    while (true)
    {
        ...
        md->set_source_inline(
            ...
            metadata::DataManager::get().to_data(
                DataFormat::VM2,
                vector<uint8_t>(input.line.begin(), input.line.end())));
        if (!dest(move(md)))
            return false;
    }
    return true;
                                          ...
                                          const std::string& value) const
{
    ...
    res << setfill('0') << setw(4) << time.ye << setfill('0') << setw(2)
        << time.mo << setfill('0') << setw(2) << time.da << setfill('0')
        << setw(2) << time.ho << setfill('0') << setw(2) << time.mi;
...
void Scanner::normalize_before_dispatch(Metadata& md)
{
    if (const Value* value = md.get<types::Value>())
    {
        ...
        {
            md.set_cached_data(metadata::DataManager::get().to_data(
                DataFormat::VM2, std::move(normalized)));
            md.makeInline();
        }
    }
}
