...
MatcherType::MatcherType(const std::string& name, types::Code code,
...
MatcherType* MatcherType::find(const std::string& name)
{
    if (!matchers)
        return 0;
    std::map<std::string, MatcherType*>::const_iterator i =
        matchers->find(name);
    if (i == matchers->end())
        return 0;
    return i->second;
...
{
    vector<string> res;
    for (std::map<std::string, MatcherType*>::const_iterator i =
             matchers->begin();
         i != matchers->end(); ++i)
...
void MatcherType::register_matcher(
    const std::string& name, types::Code code,
...
bool Implementation::match_buffer(types::Code code, const uint8_t* data,
                                  unsigned size) const
{
...
bool OR::match_buffer(types::Code code, const uint8_t* data,
                      unsigned size) const
{
    if (components.empty())
        return true;
...
bool OR::intersect_interval(core::Interval& interval) const
{
    for (auto i : components)
    {
        const matcher::MatchReftime* rt =
            dynamic_cast<const matcher::MatchReftime*>(i.get());
        assert(rt != nullptr);
...
std::string OR::toReftimeSQL(const std::string& column) const
{
    ...
    {
        const matcher::MatchReftime* mr =
            dynamic_cast<const matcher::MatchReftime*>(components[0].get());
        ...
        for (const auto& i : components)
        {
            ...
            if (!mr)
                throw std::runtime_error(
                    "arkimet bug: toReftimeSQL called on non-reftime matchers");
            if (first)
                first = false;
            else
                res += " OR ";
...
unique_ptr<OR> OR::parse(const Aliases* aliases, const MatcherType& mt,
                         const std::string& pattern)
{
    ...
    for (Splitter::const_iterator i = splitter.begin(pattern);
         i != splitter.end(); ++i)
    {
        if (shared_ptr<OR> exprs =
                aliases ? aliases->get(str::lower(*i)) : nullptr)
...
AND* AND::clone() const
{
    ...
    for (const auto& c : components)
        res->components.emplace(
            std::make_pair(c.first, std::shared_ptr<OR>(c.second->clone())));
    return res.release();
}

std::string AND::name() const { return "matcher"; }
...
void AND::merge(const AND& o)
{
    ...
    {
        if (i2 == o.components.end() ||
            (i1 != components.end() && i1->first < i2->first))
        {
        ...
        else if (i1 == components.end() ||
                 (i2 != o.components.end() && i1->first > i2->first))
        {
...
bool AND::match_buffer(types::Code code, const uint8_t* data,
                       unsigned size) const
{
    if (empty())
        return true;
...
void AND::split(const std::set<types::Code>& codes, AND& with,
                AND& without) const
{
    for (const auto& i : components)
    {
...
unique_ptr<AND> AND::parse(const AliasDatabase& aliases,
                           const std::string& pattern)
{
    ...
    for (Tokenizer::const_iterator i = tok.begin(); i != tok.end(); ++i)
    {
        ...
        if (pos == string::npos)
            throw std::invalid_argument("cannot parse matcher subexpression '" +
                                        expr +
        ...
        if (mt == 0)
            throw std::invalid_argument(
                "cannot parse matcher subexpression: unknown match type: '" +
                type + "'");

        res->components.insert(
            make_pair(mt->code, OR::parse(aliases.get(type), *mt, patterns)));
    }
...
std::unique_ptr<AND> AND::for_interval(const core::Interval& interval)
{
    ...
    reftime->tests.push_back(
        matcher::reftime::DTMatch::createInterval(interval));

    std::unique_ptr<AND> res(new AND);
    res->components.insert(
        make_pair(TYPE_REFTIME, OR::wrap(std::move(reftime))));
...
uint32_t OptionalCommaList::getUnsignedWithMissing(size_t pos, uint32_t missing,
                                                   bool& has_val) const
{
    if ((has_val = has(pos)))
    {
...
Optional<uint32_t>
OptionalCommaList::getUnsignedWithMissing(size_t pos, uint32_t missing) const
{
    if (!has(pos))
...
const std::string& OptionalCommaList::getString(size_t pos,
