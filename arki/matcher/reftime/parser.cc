...
struct DateInterval : public DTMatch
{
    ...
    DateInterval(const core::Interval& interval) : interval(interval) {}
        ...
        if (interval.begin.is_set())
        {
            if (interval.end.is_set())
                res << "(" << column << ">='" << interval.begin.to_sql()
                    << "' AND " << column << "<'" << interval.end.to_sql()
                    << "')";
            else
                res << column << ">='" << interval.begin.to_sql() << "'";
        }
...
struct TimeLE : public TimeMatch
{
    ...
    bool match(const core::Time& tt) const override
    {
        return time_to_seconds(tt) <= ref;
    }
    bool match(const core::Interval& interval) const override
    {
        if (Time::duration(interval) >= 3600 * 24)
            return true;
        if (time_to_seconds(interval.begin) <= ref)
            return true;
        if (time_to_seconds(interval.end) <= ref)
            return true;
        return false;
    }
    string sql(const std::string& column) const override
    {
        return "TIME(" + column + ")<=" + tosqlTime(ref);
    }
...
struct TimeEQ : public DTMatch
{
    ...
    bool match(const core::Interval& interval) const override
    {
        // If the times are more than 24hours apart, then we necessarily cover a
        // whole day
        if (Time::duration(interval) >= 3600 * 24)
            return true;
    ...
    string sql(const std::string& column) const override
    {
        if (geref == leref)
            ...
            return "(TIME(" + column + ")>=" + tosqlTime(geref) + " AND TIME(" +
                   column + ")<=" + tosqlTime(leref) + ")";
    }
...
{
    if (timebase == -1)
        timebase = time_to_seconds(tt->lowerbound());
...
DTMatch* Parser::createEQ(FuzzyTime* tt)
{
    ...
    if (timebase == -1)
        timebase = time_to_seconds(geref);
