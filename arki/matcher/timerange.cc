...
static Optional<INT> parseValueWithUnit(const std::string& str,
                                        types::timerange::GRIB1Unit& u)
{
    if (str.empty())
        return Optional<INT>();
    ...
    {
        std::stringstream ss;
        ss << "cannot parse timerange match expression '" << str
           << "': unknown time suffix '" << unit
           << "': valid ones are 's', 'm', 'h', 'd', 'mo', 'y'";
...
static int parseTimedefValueWithUnit(const std::string& str, bool& is_second)
{
    ...
    if (!Timedef::timeunit_parse(s, unit, val) || *s)
        throw_consistency_error("parsing 'timerange' match expression",
...
MatchTimerangeGRIB1::MatchTimerangeGRIB1(types::timerange::GRIB1Unit unit,
                                         const Optional<int>& ptype,
                                         const Optional<int>& p1,
    ...
    : unit(types::timerange::SECOND)
{
    ...
    const types::timerange::GRIB1Unit missingUnit =
        (types::timerange::GRIB1Unit)-1;
...
bool MatchTimerangeGRIB1::match_data(int mtype, int munit, int mp1, int mp2,
                                     bool use_p1, bool use_p2) const
{
...
{
    const types::timerange::GRIB1* v =
        dynamic_cast<const types::timerange::GRIB1*>(&o);
    if (!v)
        return false;
    int mtype, mp1, mp2;
...
bool MatchTimerangeGRIB1::match_buffer(types::Code code, const uint8_t* data,
                                       unsigned size) const
{
    if (code != TYPE_TIMERANGE)
        return false;
    if (size < 1)
        return false;
    if (Timerange::style(data, size) != timerange::Style::GRIB1)
        return false;
    int mtype, mp1, mp2;
    types::timerange::GRIB1Unit munit;
    bool use_p1, use_p2;
    Timerange::get_GRIB1_normalised(data, size, mtype, munit, mp1, mp2, use_p1,
                                    use_p2);
    return match_data(mtype, munit, mp1, mp2, use_p1, use_p2);
}
...
{
    const types::timerange::GRIB2* v =
        dynamic_cast<const types::timerange::GRIB2*>(&o);
    if (!v)
        return false;
...
bool MatchTimerangeGRIB2::match_buffer(types::Code code, const uint8_t* data,
                                       unsigned size) const
{
    if (code != TYPE_TIMERANGE)
        return false;
    if (size < 1)
        return false;
    if (Timerange::style(data, size) != timerange::Style::GRIB2)
        return false;
    unsigned vtype, vunit;
    signed long vp1, vp2;
...
MatchTimerangeBUFR::MatchTimerangeBUFR(const Optional<unsigned>& forecast,
                                       bool is_seconds)
...
{
    const types::timerange::BUFR* v =
        dynamic_cast<const types::timerange::BUFR*>(&o);
    if (!v)
        return false;
    ...
    if (forecast.value == 0)
        return vvalue == 0;
    if (is_seconds != types::timerange::BUFR::is_seconds(vunit))
        return false;
    if (is_seconds)
...
bool MatchTimerangeBUFR::match_buffer(types::Code code, const uint8_t* data,
                                      unsigned size) const
{
    if (code != TYPE_TIMERANGE)
        return false;
    if (size < 1)
        return false;
    if (Timerange::style(data, size) != timerange::Style::BUFR)
        return false;
    unsigned vunit, vvalue;
...
MatchTimerangeTimedef::MatchTimerangeTimedef(const Optional<int>& step,
                                             bool step_is_seconds,
                                             const Optional<int>& proc_type,
                                             const Optional<int>& proc_duration,
                                             bool proc_duration_is_seconds)
    : step(step), step_is_seconds(step_is_seconds), proc_type(proc_type),
      proc_duration(proc_duration),
      proc_duration_is_seconds(proc_duration_is_seconds)
{
}
...
MatchTimerangeTimedef::MatchTimerangeTimedef(const std::string& pattern)
{
    ...
    if (args.has(2))
    {
        if (args[2] == "-")
        {
            ...
            proc_duration.set(
                parseTimedefValueWithUnit(args[2], proc_duration_is_seconds));
    }
}

MatchTimerangeTimedef* MatchTimerangeTimedef::clone() const
{
    return new MatchTimerangeTimedef(step, step_is_seconds, proc_type,
                                     proc_duration, proc_duration_is_seconds);
}
...
bool MatchTimerangeTimedef::matchItem(const Type& o) const
{
    ...
    if (proc_duration.present)
    {
        ...
        if (proc_duration.value != v_duration ||
            proc_duration_is_seconds != v_is_seconds)
            return false;
    }
    return true;
}
...
Implementation* MatchTimerange::parse(const std::string& pattern)
{
    ...
    {
        case types::Timerange::Style::GRIB1:
            return new MatchTimerangeGRIB1(rest);
        case types::Timerange::Style::GRIB2:
            return new MatchTimerangeGRIB2(rest);
        case types::Timerange::Style::TIMEDEF:
            return new MatchTimerangeTimedef(rest);
        case types::Timerange::Style::BUFR: return new MatchTimerangeBUFR(rest);
        default:
            throw std::invalid_argument("cannot parse type of timerange to "
                                        "match: unsupported timerange style: " +
                                        name);
    }
}
...
void MatchTimerange::init()
{
    MatcherType::register_matcher(
        "timerange", TYPE_TIMERANGE,
        (MatcherType::subexpr_parser)MatchTimerange::parse);
}
