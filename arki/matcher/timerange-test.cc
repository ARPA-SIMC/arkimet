...
void Tests::register_tests()
{
    ...
    add_method("timedef_parse", [] {
        auto parse = [](const char* str) {
            ...
            return std::unique_ptr<matcher::MatchTimerange>(
                dynamic_cast<matcher::MatchTimerange*>(impl));
        };
        {
            auto matcher = parse("Timedef,+72h,1,6h");
            const matcher::MatchTimerangeTimedef* m =
                dynamic_cast<const matcher::MatchTimerangeTimedef*>(
                    matcher.get());
            wassert_true(m);
    ...
    add_method("timedef_grib2", [] {
        ...
        // timerange:GRIB2 metadata items do not carry enough information to
        // feed Timedef matchers
        ensure_matches("timerange:Timedef", md);
        ensure_matches("timerange:Timedef,-", md);
        ensure_matches("timerange:Timedef,-,-", md);
        ensure_matches("timerange:Timedef,-,-,-", md);
    ...
    add_method("serialize", [] {
        ...
        wassert(actual(m.toStringExpanded()) ==
                "timerange:GRIB1,6,3600s,7200s");
    });
}
