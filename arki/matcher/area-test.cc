...
void Tests::register_tests()
{
    ...
    add_method("grib", [] {
        ...
        // 0x1ffffff = 33554431
        ValueBag testArea2 =
            ValueBag::parse("foo=15, bar=15000, baz=-1200, moo=33554431, "
                            "antani=0, blinda=-1, supercazzola=-7654321");
    ...
    add_method("bbox_equals", [] {
        ...
        Metadata md;
        arki::tests::fill(md);
        md.test_set(types::Area::decodeString(
            "GRIB(Ni=441, Nj=181, latfirst=45000000, latlast=43000000, "
            "lonfirst=10000000, lonlast=12000000, type=0)"));
        ensure_matches(
            "area: bbox equals POLYGON((10 43, 12 43, 12 45, 10 45, 10 43))",
            md);
        ensure_not_matches("area: bbox equals POINT EMPTY", md);
        ensure_not_matches("area: bbox equals POINT(11 44)", md);
        ensure_not_matches(
            "area: bbox equals POLYGON((12 43, 10 44, 12 45, 12 43))", md);
    });
    ...
    add_method("bbox_covers", [] {
        ...
        ensure_matches(
            "area: bbox covers LINESTRING(10 43, 10 45, 12 45, 12 43, 10 43)",
            md);
        ensure_matches(
            "area: bbox covers POLYGON((10 43, 10 45, 12 45, 12 43, 10 43))",
            md);
        ensure_matches("area: bbox covers POLYGON((10.5 43.5, 10.5 44.5, 11.5 "
                       "44.5, 11.5 43.5, 10.5 43.5))",
                       md);
        ensure_matches(
            "area: bbox covers POLYGON((12 43, 10 44, 12 45, 12 43))", md);
        ...
        ensure_not_matches(
            ...
            md);
        ensure_not_matches(
            "area: bbox covers POLYGON((12 42, 10 44, 12 45, 12 42))", md);

        // Known cases that gave trouble
        md.test_set(types::Area::decodeString(
            "GRIB(Ni=441, Nj=181, latfirst=75000000, latlast=30000000, "
            "lonfirst=-45000000, lonlast=65000000, type=0)"));
        ensure_matches(
            "area:bbox covers POLYGON((30 60, -20 60, -20 30, 30 30, 30 60))",
            md);
    });
    ...
    add_method("bbox_intersects", [] {
        ...
        ensure_matches("area: bbox intersects LINESTRING(10 43, 10 45, 12 45, "
                       "12 43, 10 43)",
                       md);
        ensure_matches("area: bbox intersects POLYGON((10 43, 10 45, 12 45, 12 "
                       "43, 10 43))",
                       md);
        ensure_matches("area: bbox intersects POLYGON((10.5 43.5, 10.5 44.5, "
                       "11.5 44.5, 11.5 43.5, 10.5 43.5))",
                       md);
        ensure_matches(
            "area: bbox intersects POLYGON((12 43, 10 44, 12 45, 12 43))", md);
        ensure_matches("area: bbox intersects POLYGON((10 43, 10 45, 13 45, 13 "
                       "43, 10 43))",
                       md);
        ensure_matches(
            "area: bbox intersects POLYGON((12 42, 10 44, 12 45, 12 42))", md);
        ensure_matches(
            "area: bbox intersects POLYGON((9 40, 10 43, 10 40, 9 40))",
            md); // Touches one vertex
        ...
        ensure_not_matches(
            "area: bbox intersects LINESTRING(9 40, 10 42, 10 40, 9 40)",
            md); // Disjoint
        ensure_not_matches(
            "area: bbox intersects POLYGON((9 40, 10 42, 10 40, 9 40))",
            md); // Disjoint
    });
    ...
    add_method("bbox_coveredby", [] {
        ...
        md.test_set(types::Area::decodeString(
            ...
            "lonfirst=10000000, lonlast=12000000, type=0)"));
        ensure_matches(
            "area: bbox coveredby POLYGON((10 43, 10 45, 12 45, 12 43, 10 43))",
            md); // Same shape
        ensure_matches(
            "area: bbox coveredby POLYGON((10 43, 10 45, 13 45, 13 43, 10 43))",
            md); // Trapezoid containing area
        ensure_not_matches("area: bbox coveredby POINT(11 44)",
                           md); // Intersection exists but area is 0
        ensure_not_matches("area: bbox coveredby POINT(12 43)",
                           md); // Intersection exists but area is 0
        ...
        ensure_not_matches(
            "area: bbox coveredby POLYGON((12 42, 10 44, 12 45, 12 42))", md);
        ensure_not_matches(
            "area: bbox coveredby POLYGON((12 43, 10 44, 12 45, 12 43))",
            md); // Triangle contained inside, touching borders (?)
        ensure_not_matches("area: bbox coveredby POLYGON((10.5 43.5, 10.5 "
                           "44.5, 11.5 44.5, 11.5 43.5, 10.5 43.5))",
                           md); // Fully inside
        ensure_not_matches("area: bbox coveredby LINESTRING(10 43, 10 45, 12 "
                           "45, 12 43, 10 43)",
                           md); // Same shape, but intersection area is 0
        ensure_not_matches(
            "area: bbox coveredby POLYGON((9 40, 10 43, 10 40, 9 40))",
            md); // Touches one vertex
        ensure_not_matches(
            "area: bbox coveredby LINESTRING(9 40, 10 42, 10 40, 9 40)",
            md); // Disjoint
        ensure_not_matches(
            "area: bbox coveredby POLYGON((9 40, 10 42, 10 40, 9 40))",
            md); // Disjoint

        md.test_set(types::Area::decodeString(
            "GRIB(blo=0, lat=4502770, lon=966670, sta=101)"));
        ensure_matches(
            "area:bbox coveredby POLYGON((9 45, 10 45, 10 46, 9 46, 9 45))",
            md); // Point contained
    });
    ...
    add_method("odimh5", [] {
        ...
        Metadata md;
        md.test_set(types::Area::decodeString(
            "ODIMH5(foo=5,bar=5000,baz=-200,blinda=0,pippo=\"pippo\",pluto="
            "\"12\",aaa=0,zzz=1)"));
    ...
    add_method("odimh5_octagon", [] {
        // Vediamo se la formula per calcolare un ottagono con centro e raggio
        // del radar funziona
        Metadata md1;
        md1.test_set(types::Area::decodeString(
            "ODIMH5(lon=11623600,lat=44456700,radius=100000)"));
    ...
    add_method("vm2", [] {
        ...
        auto e1 = wassert_throws(std::invalid_argument,
                                 parser.parse("area:VM2,ciccio=riccio"));
