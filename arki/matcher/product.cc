...
bool MatchProductGRIB1::matchItem(const Type& o) const
{
    ...
    if (!v)
        return false;
    if (v->style() != product::Style::GRIB1)
        return false;
    unsigned ori, tab, pro;
    v->get_GRIB1(ori, tab, pro);
    if (origin != -1 && (unsigned)origin != ori)
        return false;
    if (table != -1 && (unsigned)table != tab)
        return false;
    if (product != -1 && (unsigned)product != pro)
        return false;
    return true;
}

bool MatchProductGRIB1::match_buffer(types::Code code, const uint8_t* data,
                                     unsigned size) const
{
    if (code != TYPE_PRODUCT)
        return false;
    if (size < 1)
        return false;
    if (Product::style(data, size) != product::Style::GRIB1)
        return false;
    unsigned ori, tab, pro;
...
MatchProductGRIB2::MatchProductGRIB2(int centre, int discipline, int category,
                                     int number, int table_version,
...
{
    return new MatchProductGRIB2(centre, discipline, category, number,
...
bool MatchProductGRIB2::matchItem(const Type& o) const
{
    ...
    if (!v)
        return false;
    if (v->style() != product::Style::GRIB2)
        return false;
    unsigned ce, di, ca, nu, ta, lo;
    v->get_GRIB2(ce, di, ca, nu, ta, lo);
    if (centre != -1 && (unsigned)centre != ce)
        return false;
    if (discipline != -1 && (unsigned)discipline != di)
        return false;
    if (category != -1 && (unsigned)category != ca)
        return false;
    if (number != -1 && (unsigned)number != nu)
        return false;
    if (table_version != -1 && (unsigned)table_version != ta)
        return false;
    if (local_table_version != -1 && (unsigned)local_table_version != lo)
        return false;
    return true;
}

bool MatchProductGRIB2::match_buffer(types::Code code, const uint8_t* data,
                                     unsigned size) const
{
    if (code != TYPE_PRODUCT)
        return false;
    if (size < 1)
        return false;
    if (Product::style(data, size) != product::Style::GRIB2)
        return false;
    unsigned ce, di, ca, nu, ta, lo;
...
std::string MatchProductGRIB2::toString() const
{
    ...
    if (number != -1)
        ...
        res.addUndef();
    if (table_version != -1)
        res.add(table_version);
    else
        res.addUndef();
    if (local_table_version != -1)
        res.add(local_table_version);
    else
        res.addUndef();
    return res.join();
}

MatchProductBUFR::MatchProductBUFR(int type, int subtype, int localsubtype,
...
bool MatchProductBUFR::matchItem(const Type& o) const
{
    ...
    if (!v)
        return false;
    if (v->style() != product::Style::BUFR)
        return false;
    unsigned ty, su, lo;
    ValueBag va;
    v->get_BUFR(ty, su, lo, va);
    if (type != -1 && (unsigned)type != ty)
        return false;
    if (subtype != -1 && (unsigned)subtype != su)
        return false;
    if (localsubtype != -1 && (unsigned)localsubtype != lo)
        return false;
    return values.is_subset(va);
}

bool MatchProductBUFR::match_buffer(types::Code code, const uint8_t* data,
                                    unsigned size) const
{
    if (code != TYPE_PRODUCT)
        return false;
    if (size < 1)
        return false;
    if (Product::style(data, size) != product::Style::BUFR)
        return false;
    unsigned ty, su, lo;
    ValueBag va;
...
MatchProductODIMH5::MatchProductODIMH5(const std::string& obj,
...
bool MatchProductODIMH5::matchItem(const Type& o) const
{
    ...
    if (!v)
        return false;
    if (v->style() != product::Style::ODIMH5)
        return false;
    std::string ob, pr;
    v->get_ODIMH5(ob, pr);
    if (obj.size() && obj != ob)
        return false;
    if (prod.size() && prod != pr)
        return false;
    return true;
}

bool MatchProductODIMH5::match_buffer(types::Code code, const uint8_t* data,
                                      unsigned size) const
{
    if (code != TYPE_PRODUCT)
        return false;
    if (size < 1)
        return false;
    if (Product::style(data, size) != product::Style::ODIMH5)
        return false;
    std::string ob, pr;
...
std::string MatchProductODIMH5::toString() const
{
    ...
    if (prod.size())
        ...
        res.addUndef();
    /*REMOVED:if (!isnan(prodpar1)) 	res.add(prodpar1); 	else
     * res.addUndef();*/
    /*REMOVED:if (!isnan(prodpar2)) 	res.add(prodpar2); 	else
     * res.addUndef();*/
    return res.join();
}

MatchProductVM2::MatchProductVM2(int variable_id,
                                 const types::ValueBagMatcher& expr,
...
bool MatchProductVM2::matchItem(const Type& o) const
{
    ...
    if (!v)
        return false;
    if (v->style() != product::Style::VM2)
        return false;
    unsigned vi;
    v->get_VM2(vi);

    if (variable_id != -1 && (unsigned)variable_id != vi)
        return false;
    if (!expr.empty() &&
...
bool MatchProductVM2::match_buffer(types::Code code, const uint8_t* data,
                                   unsigned size) const
{
    if (code != TYPE_PRODUCT)
        return false;
    if (size < 1)
        return false;
    if (Product::style(data, size) != product::Style::VM2)
        return false;
    unsigned vi;
...
Implementation* MatchProduct::parse(const std::string& pattern)
{
    ...
    switch (types::Product::parseStyle(name))
    {
        ...
        default:
            throw std::invalid_argument("cannot parse type of product to "
                                        "match: unsupported product style: " +
                                        name);
    }
}
...
void MatchProduct::init()
{
    MatcherType::register_matcher(
        ...
        (MatcherType::subexpr_parser)MatchProduct::parse);
}
