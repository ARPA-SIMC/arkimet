...
// Metadata Scan Order
const types::Code Table::mso[] = {TYPE_ORIGIN,    TYPE_PRODUCT, TYPE_LEVEL,
                                  TYPE_TIMERANGE, TYPE_AREA,    TYPE_PRODDEF,
                                  TYPE_BBOX,      TYPE_RUN,     TYPE_QUANTITY,
                                  TYPE_TASK
...
bool Row::matches(const Matcher& matcher) const
{
    ...
    for (unsigned i = 0; i < mso_size; ++i)
    {
        ...
        if (!items[i])
            return false;
        if (!item_matcher->matchItem(*items[i]))
            return false;
    }
...
Table::Table() : interns(new TypeIntern[Table::msoSize])
{
    buildMsoSerLen();
    buildItemMsoMap();
}

Table::~Table() { delete[] interns; }
...
bool Table::equals(Table& table)
{
    ...
    for (unsigned ri = 0; ri < rows.size(); ++ri)
    {
        ...
        for (unsigned ci = 0; ci < msoSize; ++ci)
        {
            ...
            {
                translated.items[ci] =
                    interns[ci].lookup(*table.rows[ri].items[ci]);
                if (!translated.items[ci])
                    return false;
            }
        }
        ...
        if (*pos != translated)
            return false;
        if (pos->stats != translated.stats)
            return false;
    }
    return true;
}

const types::Type* Table::intern(unsigned pos,
                                 std::unique_ptr<types::Type>&& item)
{
    return interns[pos].intern(move(item));
}

void Table::merge(const Metadata& md) { merge(md, Stats(md)); }
...
void Table::merge(const std::vector<const types::Type*>& md, const Stats& st,
                  const vector<unsigned>& positions)
{
    Row new_row(st);
    new_row.set_to_zero();
    for (vector<unsigned>::const_iterator i = positions.begin();
         i != positions.end(); ++i)
    {
        if (*i < md.size() && md[*i])
            new_row.items[*i] = interns[*i].intern(*md[*i]);
...
void Table::merge(const structured::Keys& keys, const structured::Reader& val)
{
    ...
    val.sub(keys.summary_stats, "summary stats",
            [&](const structured::Reader& summarystats) {
    ...
    for (size_t pos = 0; pos < msoSize; ++pos)
    {
        ...
        if (!val.has_key(types::tag(code), structured::NodeType::MAPPING))
            continue;
        val.sub(types::tag(code), "summary item",
                [&](const structured::Reader& item) {
                    new_row.items[pos] = interns[pos].intern(
                        types::decode_structure(keys, code, item));
                });
    }
         ...
         i != yamlStream.end(); ++i)
    {
        ...
        switch (type)
        {
            case TYPE_SUMMARYITEM: {
                auto in =
                    LineReader::from_chars(i->second.data(), i->second.size());
                YamlStream yamlStream;
                for (YamlStream::const_iterator i = yamlStream.begin(*in);
                     i != yamlStream.end(); ++i)
                {
                    ...
                    if (pos < 0)
                        throw runtime_error("cannot parse summary item: found "
                                            "element of unsupported type " +
                                            types::formatCode(type));
                    new_row.items[pos] = interns[pos].intern(
                        types::decodeString(type, i->second));
                }
            }
            break;
...
void Table::dump(std::ostream& out) { want_clean(); }
...
types::Code Visitor::codeForPos(size_t pos) { return Table::mso[pos]; }

int Visitor::posForCode(types::Code code)
{
    if ((size_t)code >= itemMsoMapSize)
        return -1;
