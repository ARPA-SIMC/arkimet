...
static const char* relpath = "testfile.grib";
...
this->add_method("append", [](Fixture& f) {
    ...
    {
        auto w = Segment::make_writer("grib", sys::getcwd(), relpath, abspath);
        ...
        wassert(test_append_transaction_ok(w.get(), mdc[0], Segment::padding));

        // Then fail one
        wassert(test_append_transaction_rollback(w.get(), mdc[1], Segment::padding));

        // Then succeed again
        wassert(test_append_transaction_ok(w.get(), mdc[2], Segment::padding));
    }
...
this->add_method("large", [](Fixture& f) {
    ...
    metadata::TestCollection mdc("inbound/test.grib1");
    {
        ...
        Segment::make_checker("grib", sys::getcwd(), relpath, abspath)->test_truncate(0x7FFFFFFF);
        wassert(actual(sys::size(relpath)) == 0x7FFFFFFFu);
    }

    {
        auto dw = Segment::make_writer("grib", sys::getcwd(), relpath, abspath);

        // Try a successful transaction
        wassert(test_append_transaction_ok(dw.get(), mdc[0], Segment::padding));

        // Then fail one
        wassert(test_append_transaction_rollback(dw.get(), mdc[1], Segment::padding));

        // Then succeed again
        wassert(test_append_transaction_ok(dw.get(), mdc[2], Segment::padding));
    }

    wassert(actual(sys::size(relpath)) == 0x7FFFFFFFu + mdc[0].data_size() + mdc[2].data_size() + 2 * Segment::padding);
