...
void Aggregate::init_select_by_id() const
{
    ...
    q_select_by_id.compile("SELECT " + query + " FROM " + m_table_name +
                           " WHERE id=?");
}
...
void Aggregate::init_insert() const
{
    ...
    q_insert.compile("INSERT INTO " + m_table_name + " (" + names +
                     ") VALUES (" + placeholders + ")");
}
...
void Aggregate::read(int id, Metadata& md) const
{
    ...
    size_t idx = 0;
    for (Attrs::const_iterator j = m_attrs.begin(); j != m_attrs.end();
         ++j, ++idx)
    {
        int id = i->second[idx];
        if (id != -1)
...
int Aggregate::add_constraints(const Matcher& m,
                               ...
                               const std::string& prefix) const
{
    ...
    for (Attrs::const_iterator i = m_attrs.begin(); i != m_attrs.end(); ++i)
    {
        ...
        constraints.push_back(prefix + "." + (*i)->name + " " +
                              fmtin((*i)->query(*mi)));
        ++found;
    }
    return found;
...
std::string Aggregate::make_subquery(const Matcher& m) const
{
    ...
    for (Attrs::const_iterator i = m_attrs.begin(); i != m_attrs.end(); ++i)
    {
        ...
        // We found something: generate a constraint for it
        constraints.push_back((*i)->name + " " + fmtin((*i)->query(*mi)));
    }
    if (constraints.empty())
        return std::string();
    return "SELECT id FROM " + m_table_name + " WHERE " +
...
void Aggregate::initDB(const std::set<types::Code>& components_indexed)
{
    ...
    for (Attrs::const_iterator i = m_attrs.begin(); i != m_attrs.end(); ++i)
    {
        if (components_indexed.find((*i)->code) == components_indexed.end())
            continue;
        string name = (*i)->name;
        m_db.exec("CREATE INDEX IF NOT EXISTS " + m_table_name + "_idx_" +
                  name + " ON " + m_table_name + " (" + name + ")");
    }
}
