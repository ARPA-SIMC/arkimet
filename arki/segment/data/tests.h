...
// TODO: virtual Pending repack(const std::string& rootdir, const std::string&
// relpath, metadata::Collection& mds) = 0;

void test_append_transaction_ok(segment::data::Writer* dw, Metadata& md,
                                int append_amount_adjust = 0);
...
inline ActualSegment actual_segment(const std::string& path)
{
    return ActualSegment(path);
}
