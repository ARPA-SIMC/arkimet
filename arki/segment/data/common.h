...
struct AppendCreator
{
    ...
    virtual std::unique_ptr<types::Source> create_source(const Metadata& md,
                                                         const Span& span);

    /// Append data to the segment, returning the offset at which it has been
    /// written
...
struct AppendCheckBackend
{
    ...
    virtual size_t
    compute_unindexed_space(const std::vector<Span>& indexed_spans) const;
