...
void SegmentTests<Data, FixtureData>::register_tests()
{
    ...
    this->add_method("read", [](Fixture& f) {
        ...
        for (auto& md : f.seg_mds)
        {
            std::vector<uint8_t> buf =
                wcallchecked(reader->read(md->sourceBlob()));
            wassert(actual(buf.size()) == md->sourceBlob().size);

            {
                auto stream = StreamOutput::create(std::make_shared<sys::File>(
                    "stream.out", O_WRONLY | O_CREAT | O_TRUNC));
                wassert(actual(reader->stream(md->sourceBlob(), *stream)) ==
                        stream::SendResult());
            ...
            // wassert_true(std::equal(buf.begin(), buf.end(), buf1.begin(),
            // buf1.end()));
        }
    });
    ...
    this->add_method("check", [](Fixture& f) {
        ...
        wassert(actual(checker->check(rep, f.seg_mds)) == segment::SEGMENT_OK);
        wassert(actual(checker->check(rep, f.seg_mds, true)) ==
                segment::SEGMENT_OK);
        ...
        // Simulate one element being deleted
        {
            ...
            wassert(actual(checker->check(rep, mdc1)) ==
                    segment::SEGMENT_DIRTY);
        }
        {
            metadata::Collection mdc1;
        ...
        // Simulate corrupted file
        {
            ...
            mdc1.push_back(f.seg_mds[1]);
            mdc1.push_back(f.seg_mds[2]);
            std::unique_ptr<types::source::Blob> src(
                f.seg_mds[0].sourceBlob().clone());
            src->offset += 1;
            mdc1[0].set_source(std::unique_ptr<types::Source>(src.release()));
            wassert(actual(checker->check(rep, mdc1, true)) ==
                    segment::SEGMENT_CORRUPTED);
            wassert(actual(checker->check(rep, mdc1, false)) ==
                    segment::SEGMENT_CORRUPTED);
        }
    ...
    this->add_method("issue244", [](Fixture& f) {
        ...
        auto data = arki::metadata::DataManager::get().to_data(f.td.format,
                                                               std::move(buf));
        ...
        {
            auto stream = StreamOutput::create(std::make_shared<sys::File>(
                "stream.out", O_WRONLY | O_TRUNC | O_APPEND));
            wassert(actual(reader->stream(md->sourceBlob(), *stream)) ==
                    stream::SendResult());
            size_t pad_size = f.td.format == DataFormat::VM2 ? 1 : 0;
            size_t size     = sys::size("stream.out");
