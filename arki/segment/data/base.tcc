    ...
    const std::filesystem::path& new_relpath)
{
    ...
    auto new_segment = segment_session->segment_from_relpath_and_format(
        new_relpath, segment().format());

    // Sanity checks: avoid conflicts
    if (std::filesystem::exists(new_segment->abspath()) ||
        std::filesystem::exists(
            sys::with_suffix(new_segment->abspath(), ".tar")) ||
        std::filesystem::exists(
            sys::with_suffix(new_segment->abspath(), ".gz")) ||
        std::filesystem::exists(
            sys::with_suffix(new_segment->abspath(), ".zip")))
    {
        std::stringstream ss;
        ss << "cannot move " << this->segment().abspath() << " to "
           << new_segment->abspath()
    ...
    auto target_metadata =
        sys::with_suffix(new_segment->abspath(), ".metadata");
    ...
    sys::rename_ifexists(
        sys::with_suffix(this->segment().abspath(), ".metadata"),
        target_metadata);
    sys::rename_ifexists(
