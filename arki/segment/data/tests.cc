...
SegmentTest::SegmentTest()
    : format("grib"), root("."), relpath("testfile.grib"),
      abspath(std::filesystem::canonical(relpath)), mdc("inbound/test.grib1")
{
}
...
void SegmentRemoveTest::run()
{
    ...
    wassert(actual(segment->data().exists_on_disk()).istrue());

    wassert(actual(segment->remove()) >= 42000u);

    wassert(actual(segment->data().exists_on_disk()).isfalse());
}

void test_append_transaction_ok(segment::data::Writer* dw, Metadata& md,
                                int append_amount_adjust)
{
    ...
    wassert(actual(sys::size(dw->segment().abspath())) ==
            orig_fsize + data_size + append_amount_adjust);
    ...
    wassert(
        actual_type(md.source())
            .is_source_blob(md.source().format, std::filesystem::current_path(),
                            dw->segment().relpath(), orig_fsize, data_size));
}

void test_append_transaction_rollback(segment::data::Writer* dw, Metadata& md,
                                      int append_amount_adjust)
{
    // Make a snapshot of everything before appending
    unique_ptr<Source> orig_source(md.source().clone());
    ...
    const types::source::Blob& new_source = dw->append(md);
    wassert(actual((size_t)new_source.offset) == orig_fsize);
    wassert(actual(sys::size(dw->segment().abspath(), 0)) ==
            orig_fsize + new_source.size + append_amount_adjust);
    wassert(actual_type(md.source()) == *orig_source);
...
{
    bool exists = std::filesystem::exists(_actual) ||
                  std::filesystem::exists(_actual + ".gz") ||
                  std::filesystem::exists(_actual + ".tar") ||
                  std::filesystem::exists(_actual + ".zip");
    if (!exists)
        throw TestFailed(_actual +
                         " does not exist (tried also .gz, .tar, and .zip)");
}

void ActualSegment::exists(const std::vector<std::string>& extensions)
{
    for (auto ext :
         {"", ".gz", ".tar", ".zip", ".gz.idx", ".metadata", ".summary"})
