...
void make_samples()
{
    // FIXME: move these into a subdir, which makes setup/cleanup operations
    // easier
...
void Tests::register_tests()
{
    ...
    add_method("auto_instantiate_existing", [] {
        ...
        wassert(actual(get_writer("grib", "testfile.grib")->data().type()) ==
                "concat");
        wassert(actual(get_writer("bufr", "testfile.bufr")->data().type()) ==
                "concat");
        wassert(actual(get_writer("vm2", "testfile.vm2")->data().type()) ==
                "lines");
        wassert_throws(std::runtime_error, get_writer("odimh5", "testfile.h5"));
        wassert(actual(get_writer("grib", "testdir.grib")->data().type()) ==
                "dir");
        wassert(actual(get_writer("bufr", "testdir.bufr")->data().type()) ==
                "dir");
        wassert(actual(get_writer("vm2", "testdir.vm2")->data().type()) ==
                "dir");
        wassert(actual(get_writer("odimh5", "testdir.h5")->data().type()) ==
                "dir");
        ...
        wassert(actual(get_checker("grib", "testfile.grib")->data().type()) ==
                "concat");
        wassert(actual(get_checker("bufr", "testfile.bufr")->data().type()) ==
                "concat");
        wassert(actual(get_checker("vm2", "testfile.vm2")->data().type()) ==
                "lines");
        // wassert(actual(get_checker("odimh5", "testfile.h5")->data().type())
        // == "dir");
        wassert_throws(std::runtime_error,
                       get_checker("odimh5", "testfile.h5"));
        wassert(actual(get_checker("grib", "testdir.grib")->data().type()) ==
                "dir");
        wassert(actual(get_checker("bufr", "testdir.bufr")->data().type()) ==
                "dir");
        wassert(actual(get_checker("vm2", "testdir.vm2")->data().type()) ==
                "dir");
        wassert(actual(get_checker("odimh5", "testdir.h5")->data().type()) ==
                "dir");
        wassert(actual(get_checker("grib", "testtar.grib")->data().type()) ==
                "tar");
        wassert(actual(get_checker("bufr", "testtar.bufr")->data().type()) ==
                "tar");
        wassert(actual(get_checker("vm2", "testtar.vm2")->data().type()) ==
                "tar");
        wassert(actual(get_checker("odimh5", "testtar.h5")->data().type()) ==
                "tar");
    });
}
